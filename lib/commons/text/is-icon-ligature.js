...
text.isIconLigature = function(
	...
	// NOTE: IE does not support `.some` on Unit8/32Arrays
	if (!Array.from(compareData).some(pixel => pixel)) {
		font.numLigatures++;
		return true;
	}
	...
	// NOTE: IE does not support `.reduce` on Unit32Arrays
	const differences = Array.from(compareData).reduce((diff, pixel, i) => {
