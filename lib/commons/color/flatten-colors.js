...
const blendFunctions = {
  ...
  overlay(Cb, Cs) {
    ...
    return Cb < 128 ? this.multiply(Cs, 2 * Cb) : this.screen(Cs, 2 * Cb - 255);
  },
  darken(Cb, Cs) {
  ...
  lighten(Cb, Cs) {
    ...
    return Math.round(Math.max(Cb, Cs));
  },
  'color-dodge'(Cb, Cs) {
  ...
  'hard-light'(Cb, Cs) {
    // @see https://www.w3.org/TR/compositing-1/#blendinghardlight
    return Cs < 128 ? this.multiply(Cb, 2 * Cs) : this.screen(Cb, 2 * Cs - 255);
  },
  'soft-light'(Cb, Cs) {
