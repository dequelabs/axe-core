...
import { getNodeFromTree, performanceTimer } from '../../core/utils';
...
let getBackgroundStackStart;
let getVisibleChildTextRectsStart;
let getTextShadowColorsStart;
let elementHasImageStart;
let getOwnBackgroundColorStart;
let fullyEncompassesStart;
let getStackingContextStart;
let getPageBackgroundColorsStart;
let flattenColorsStart;

let getBackgroundStackTime = 0;
let getVisibleChildTextRectsTime = 0;
let getTextShadowColorsTime = 0;
let elementHasImageTime = 0;
let getOwnBackgroundColorTime = 0;
let fullyEncompassesTime = 0;
let getStackingContextTime = 0;
let getPageBackgroundColorsTime = 0;
let flattenColorsTime = 0;

function _getBackgroundColor(elm, bgElms, shadowOutlineEmMax) {
  getBackgroundStackStart = window.performance.now();
  const elmStack = getBackgroundStack(elm);
  getBackgroundStackTime += window.performance.now() - getBackgroundStackStart;
  if (!elmStack) {
    return null;
  }
  ...
  for (let i = 0; i < elmStack.length; i++) {
    ...
    elementHasImageStart = window.performance.now();
    if (elementHasImage(bgElm, bgElmStyle)) {
      elementHasImageTime += window.performance.now() - elementHasImageStart;
      bgElms.push(bgElm);
    ...
    let bgColor;
    try {
      getOwnBackgroundColorStart = window.performance.now();
      bgColor = getOwnBackgroundColor(bgElmStyle, true);
      getOwnBackgroundColorTime += window.performance.now() - getOwnBackgroundColorStart;
      if (bgColor.alpha === 0) {
        continue;
      }
    } catch (error) {
    ...
    fullyEncompassesStart = window.performance.now();
    if (
      bgElmStyle.getPropertyValue('display') !== 'inline' &&
      !fullyEncompasses(bgElm, textRects)
    ) {
      fullyEncompassesTime += window.performance.now() - fullyEncompassesStart;
      bgElms.push(bgElm);
  ...
  getStackingContextStart = window.performance.now();
  const stackingContext = getStackingContext(elm, elmStack);
  getStackingContextTime += window.performance.now() - getStackingContextStart;
  bgColors = stackingContext.map(stackingContextToColor).concat(bgColors);

  getPageBackgroundColorsStart = window.performance.now();
  const pageBgs = getPageBackgroundColors(
    elm,
    elmStack.includes(document.body)
  );
  getPageBackgroundColorsTime += window.performance.now() - getPageBackgroundColorsStart;
  ...
  flattenColorsStart = window.performance.now();
  ...
  const res = flattenColors(
    blendedColor.color instanceof Color ? blendedColor.color : blendedColor,
    new Color(255, 255, 255, 1)
  );
  flattenColorsTime += window.performance.now() - flattenColorsStart;

  return res;
}

getBackgroundColor._logTime = function() {
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getBackgroundStack took ' + getBackgroundStackTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getVisibleChildTextRects took ' + getVisibleChildTextRectsTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getTextShadowColors took ' + getTextShadowColorsTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_elementHasImage took ' + elementHasImageTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getOwnBackgroundColor took ' + getOwnBackgroundColorTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_fullyEncompasses took ' + fullyEncompassesTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getStackingContext took ' + getStackingContextTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getPageBackgroundColors took ' + getPageBackgroundColorsTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_flattenColors took ' + flattenColorsTime + 'ms');

  getBackgroundStack._logTime();
  getOwnBackgroundColor._logTime();
  getStackingContext._logTime();
}
