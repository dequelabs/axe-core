...
color.getBackgroundColor = function getBackgroundColor(
	...
	(elmStack || []).some(bgElm => {
		...
		if (
			...
			color.elementHasBackgroundImageOrGradient(bgElm, bgElmStyle)
		) {
			bgColors = null;
			bgElms.push(bgElm);
...
function sortPageBackground(elmStack) {
	...
	if (
		...
		!color.elementHasBackgroundImageOrGradient(document.documentElement) &&
...
dom.isOpaque = function(node) {
	...
	return (
		color.elementHasBackgroundImageOrGradient(node, style) ||
