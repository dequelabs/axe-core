...
color.getBackgroundColor = function getBackgroundColor(
	...
	(elmStack || []).some(bgElm => {
		...
		let bgColor = color.getNonAlphaBlendedBackgroundColor(bgElm, bgElmStyle);
		...
		if (
			...
			color.elementHasBackgroundImage(bgElm, bgElmStyle)
		) {
			bgColors = null;
			bgElms.push(bgElm);
...
function getRectStack(elm) {
	const boundingCoords = axe.utils.getCoordinatesFromRect(
	...
	let filteredArr = rects
		...
		.map(rect => {
			const coords = axe.utils.getCoordinatesFromRect(rect);
			if (coords) {
...
function sortPageBackground(elmStack) {
	...
	if (
		...
		!color.elementHasBackgroundImage(document.documentElement) &&
		color.getNonAlphaBlendedBackgroundColor(document.documentElement).alpha ===
			0
	) {
