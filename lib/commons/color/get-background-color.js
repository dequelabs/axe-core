...
color.getBackgroundColor = function getBackgroundColor(elm, bgElms = []) {
	...
	(elmStack || []).some(bgElm => {
		...
		// find scrollable region parent for bgElm?
		// bgElm = body, then what?
		// find elm parent scroll region, if it has one then compare the scroll region parent to the body? keep finding scroll regions until = body?
