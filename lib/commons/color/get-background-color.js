...
/**
 * Detect if a pseudo element is visible and overlaps its background
 * @method pseudoOverlapsElm
 * @private
 * @param {Element} bgElm
 * @param {Element} elm
 * @param {Object} elmStyle
 * @param {Object} pseudoStyle
 * @return {Boolean}
 */
function pseudoOverlapsElm(bgElm, elm, elmStyle, pseudoStyle) {
	let overlaps = false;
	// check if it's even visible, might need to check zIndex
	if (pseudoStyle.opacity > 0 && pseudoStyle.visibility === 'visible') {
		var temp = document.createElement('div');
		// should we put the entire style object on it, or hand-pick things? both have downsides...
		temp.style.cssText = `
			bottom: ${pseudoStyle.bottom};
			display: ${pseudoStyle.display};
			height: ${pseudoStyle.height};
			margin: ${pseudoStyle.margin};
			...
			z-index: ${pseudoStyle.zIndex};
		`;
		bgElm.appendChild(temp);
		overlaps = dom.visuallyOverlaps(temp.getBoundingClientRect(), elm);
		bgElm.removeChild(temp);
	}
	return overlaps;
}
/**
 * Detect a pseudo element background-color or gradient for color contrast testing
 * @method elmHasPseudoBackground
 * @private
 * @param {Element} elm
 * @param {Element} bgElm
 * @return {Boolean}
 */
function elmHasPseudoBackground(elm, bgElm) {
	var beforeBGStyle = window.getComputedStyle(bgElm, ':before');
	var afterBGStyle = window.getComputedStyle(bgElm, ':after');
	function setIncomplete(bgColorReason, pseudoElementType) {
		axe.commons.color.incompleteData.set('bgColor', bgColorReason);
		axe.commons.color.incompleteData.set('elementType', pseudoElementType);
	}
	const elmStyle = window.getComputedStyle(elm);
	const comparePattern = /gradient|url|data/g;
	// check CSS for clues. if pseudo has background but no opacity/dimensions, it won't apply
	if (
		comparePattern.test(beforeBGStyle.backgroundImage) &&
		pseudoOverlapsElm(bgElm, elm, elmStyle, beforeBGStyle)
	) {
		setIncomplete('pseudoBgGradient', ':before');
		return true;
	} else if (
		comparePattern.test(afterBGStyle.backgroundImage) &&
		pseudoOverlapsElm(bgElm, elm, elmStyle, afterBGStyle)
	) {
		setIncomplete('pseudoBgGradient', ':after');
		return true;
	}
	var beforeBGColor = new color.Color();
	beforeBGColor.parseRgbString(beforeBGStyle.backgroundColor);
	var afterBGColor = new color.Color();
	afterBGColor.parseRgbString(afterBGStyle.backgroundColor);

	if (
		beforeBGColor.alpha > 0 &&
		pseudoOverlapsElm(bgElm, elm, elmStyle, beforeBGStyle)
	) {
		setIncomplete('pseudoBgColor', ':before');
		return true;
	} else if (
		afterBGColor.alpha > 0 &&
		pseudoOverlapsElm(bgElm, elm, elmStyle, afterBGStyle)
	) {
		setIncomplete('pseudoBgColor', ':after');
		return true;
	}
	return false;
}
...
color.getBackgroundColor = function(elm, bgElms = [], noScroll = false) {
	...
	(elmStack || []).some(bgElm => {
		...
		if (
			...
			// OR if the background elm has a pseudo element gradient bg or bgcolor
			elmHasPseudoBackground(elm, bgElm)
		) {
			bgColors = null;
			bgElms.push(bgElm);
