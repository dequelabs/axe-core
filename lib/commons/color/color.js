...
let colorjsStart;
let colorReplaceStart;
let settersStart;

let colorjsTime = 0;
let colorReplaceTime = 0;
let settersTime = 0;
...
export default class Color {
   ...
   * @instance
   */
  parseString(colorString, log) {
    let color;
    const parseMap = cache.get('parseMap', () => { return {}; });

    if (parseMap[colorString]) {
      color = parseMap[colorString];
    }
    else {
      // Colorjs <v0.5.0 does not support rad or turn angle values
      // @see https://github.com/LeaVerou/color.js/issues/311
      if (log) {
        colorReplaceStart = window.performance.now();
      }
      ...
      if (log) {
        colorReplaceTime += window.performance.now() - colorReplaceStart;
      }
      ...
      try {
        ...
        if (log) {
          colorjsStart = window.performance.now();
        }
        ...
        color = new Colorjs(colorString)
          ...
          .to('srgb');
        parseMap[colorString] = color;

        if (log) {
          colorjsTime += window.performance.now() - colorjsStart;
        }
    ...
    if (log) {
      settersStart = window.performance.now();
    }
    this.r = color.r;
    this.g = color.g;
    this.b = color.b;
    ...
    if (log) {
      settersTime += window.performance.now() - settersStart;
    }
...
Color._logTime = function() {
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getOwnBackgroundColor_parseString_replace took ' + colorReplaceTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getOwnBackgroundColor_parseString_colorjs took ' + colorjsTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getOwnBackgroundColor_parseString_setters took ' + settersTime + 'ms');
}
