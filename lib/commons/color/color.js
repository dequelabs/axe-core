...
const noneRegex = /\bnone\b/gi;
// a number in scientific notation that starts a component (so not the digits
// of a hex color such as `#1e3`)
const exponentRegex = /([\s(])([-+]?(?:\d*\.)?\d+e[-+]?\d+)/gi;
...
export default class Color {
  ...
  parseString(colorString) {
    ...
    if (!color) {
      ...
      // Colorjs v0.4 does not support the `none` keyword, which browsers keep in
      // computed colors (e.g. the powerless hue of a gray mixed in oklch).
      // Outside of interpolation a missing component is treated as 0
      // @see https://www.w3.org/TR/css-color-4/#missing
      colorString = colorString.replace(noneRegex, '0');

      // Colorjs v0.4 does not parse numbers in scientific notation, which
      // Firefox uses for tiny values (e.g. `oklch(0.813377 5.96046e-8 none)`)
      colorString = colorString.replace(
        exponentRegex,
        (match, before, value) => before + Number(value).toFixed(20)
      );
