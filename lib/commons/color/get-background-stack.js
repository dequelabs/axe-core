...
let getTextElementStackStart;
let reduceToElementsBelowFloatingStart;
let sortPageBackgroundStart;
let shallowArraysEqualStart;

let getTextElementStackTime = 0;
let reduceToElementsBelowFloatingTime = 0;
let sortPageBackgroundTime = 0;
let shallowArraysEqualTime = 0;
...
export default function getBackgroundStack(node) {
  ...
  getTextElementStackStart = window.performance.now();
  const elementStack = getTextElementStack(node);
  getTextElementStackTime += window.performance.now() - getTextElementStackStart;

  const stacks = elementStack.map(stack => {
    reduceToElementsBelowFloatingStart = window.performance.now();
    stack = reduceToElementsBelowFloating(stack, node);
    reduceToElementsBelowFloatingTime += window.performance.now() - reduceToElementsBelowFloatingStart;

    sortPageBackgroundStart = window.performance.now();
    stack = sortPageBackground(stack);
    sortPageBackgroundTime += window.performance.now() - sortPageBackgroundStart;
    return stack;
  });
  ...
  for (let index = 0; index < stacks.length; index++) {
    ...
    shallowArraysEqualStart = window.performance.now();
    if (index !== 0 && !shallowArraysEqual(stack, stacks[0])) {
      shallowArraysEqualTime += window.performance.now() - shallowArraysEqualStart;
...
getBackgroundStack._logTime = function() {
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getBackgroundStack_getTextElementStack took ' + getTextElementStackTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getBackgroundStack_reduceToElementsBelowFloating took ' + reduceToElementsBelowFloatingTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getBackgroundStack_sortPageBackground took ' + sortPageBackgroundTime + 'ms');
  performanceTimer._log('Measure runchecks_color-contrast#getBackgroundColor_getBackgroundStack_shallowArraysEqual took ' + shallowArraysEqualTime + 'ms');

  getTextElementStack._logTime();
}
