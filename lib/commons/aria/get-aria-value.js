...
/**
 * Get the value of a non idref(s) ARIA attribute, property, or ElementInternal
 * @method getAriaValue
...
function getAttributeValue(vNode, attrStandard, attrName) {
  ...
  return prop && vNode.actualNode[prop] ? null : value;
}

function getPropertyValue(vNode, attrStandard) {
  const { prop } = attrStandard;
  return prop ? vNode.actualNode[prop] : null;
}
