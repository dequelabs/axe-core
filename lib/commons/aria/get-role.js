...
function resolveRole(node, { noImplicit, ...roleOptions } = {}) {
  ...
  if (hasConflictResolution(vNode)) {
    // all browser currently do not treat custom elements with internal roles as the implicit role so for now we need to not access the implicit role function for them and instead treat them as having no role
