...
aria.getRoleSegments = function(node) {
	...
	if(node.hasAttributeNS('http://www.idpf.org/2007/ops', 'type')) {
		...
		roles = roles.concat(epubRoles.map(role => 'doc-' + role));
	}
...
aria.isAllowedRole = function(tagName, role) {
	var element = axe.commons.aria.lookupTable.element[tagName];

	// check in allowed subset if given role is defined
	if (element && element.roles && element.roles.indexOf(role) === -1) {
		return false;
	}
