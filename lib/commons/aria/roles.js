...
aria.getRoleSegments = function (node) {
	...
	if (node && node.hasAttributeNS('http://www.idpf.org/2007/ops', 'type')) {
		let rawEpubRoles = node.getAttributeNS('http://www.idpf.org/2007/ops', 'type');
		let epubRoles = rawEpubRoles.split(' ');
		roles = roles.concat(epubRoles.map(role => {
			return `doc-${role}`;
		}));
	}
	return roles;
...
/**
 * Verify if a given role is an allowed ARIA subset role.
 * @method isAllowedSubsetRole
 * @param {String} tagName tag name of a node
 * @param {String} role aria role to check
 * @return {Boolean} return true/false
 * @private
 */
function isAllowedSubsetRole(tagName, role) {
	if (!axe.commons.aria.lookupTable.elementAriaAllowedSubsetRolesMap[tagName]) {
		return false;
	}
	return axe.commons.aria.lookupTable.elementAriaAllowedSubsetRolesMap[tagName].indexOf(role) > -1;
};


/**
 * Verify if a given role is an allowed ARIA role. 
 ...
 */
aria.isAllowedRole = function (node, tagName, role) {
	/**
	...
	const elementCheck = {
		a: (node, role) => {
			...
			if (node.href) {
				return isAllowedSubsetRole('aWithHref', role);
			}
			return true;
		},
		area: (node) => !(node.href),
		button: (node, role) => {
			...
			return isAllowedSubsetRole('button', role);
		},
		img: (node, role) => {
			const hasAllowedEmptyAltRole = isAllowedSubsetRole('imgWithEmptyAlt', role);
			if (node.alt) {
		...
		input: (node, role) => {
			switch (node.type) {
			case 'button':
				return isAllowedSubsetRole('inputTypeButton', role);
			case 'checkbox':
				if (role === 'button' && node.hasAttribute('aria-pressed')) {
					return true;
				}
				return isAllowedSubsetRole('inputTypeCheckbox', role);
			case 'image':
				return isAllowedSubsetRole('inputTypeImage', role);
			case 'radio':
		...
		li: (node, role) => {
			...
			if (hasImplicitListitemRole) {
				return isAllowedSubsetRole('li', role);
			}
			return true;
		},
		link: (node) => !(node.href),
		...
		svg: (node, role) => {
			...
			return isAllowedSubsetRole('svg', role);
		}
	};

	const element = axe.commons.aria.lookupTable.elementAriaAllowedRolesMap[tagName];
	...
	if (element &&
		...
		element.roles.indexOf(role) === -1) {
		out = false;
	}

	// check if tag has a custom function to check its validity against various tags
	if (out && elementCheck[tagName]) {
		out = elementCheck[tagName](node, role);
	}
