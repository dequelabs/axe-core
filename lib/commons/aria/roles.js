 ...
 */
aria.isAllowedRole = function (tagName, role) {
	const element = axe.commons.aria.lookupTable.elementAriaAllowedRolesMap[tagName];
	// check in allowed subset if given role is defined
	if (element && element.roles && Array.isArray(element.roles) && element.roles.indexOf(role) === -1) {
		return false;
	}
	return true;
};
 ...
 */
aria.isAllowedSubsetRole = function (tagName, role) {
