 ...
 * @param  {Object}  options Use `excludeAbstracts` if you don't want abstracts
 * @return {Boolean}
 */
aria.isValidRole = function(role, { disallowAbstract = false } = {}) {
	const roleDefinition = aria.lookupTable.role[role];
	if (!roleDefinition) {
		return false;
	}
	return disallowAbstract ? roleDefinition.type !== 'abstract' : true;
};
