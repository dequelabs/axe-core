...
/**
 * validate if a given role is an allowed ARIA subset role.
 * @method validateAriaAllowedSubsetRole
 * @param {String} tagName tag name of a node
 * @param {String} role aria role to check
 * @return {Boolean} return true/false
 * @private
 */
function validateAriaAllowedSubsetRole(tagName, role) {
...
function validateAllowedAriaRole(node, tagName, role) {
	 ...
	 */
	const elementCheck = {
		//TODO:jey may be hoist this to lookupTable?
		a: (node, role) => {
			if (node.namespaceURI === 'http://www.w3.org/2000/svg') {
				return true;
			}
			if (node.href) {
				return validateAriaAllowedSubsetRole('aWithHref', role);
			}
			return true;
		},
		area: node => !node.href,
		button: (node, role) => {
			...
			return validateAriaAllowedSubsetRole('button', role);
		},
		img: (node, role) => {
			const hasAllowedEmptyAltRole = validateAriaAllowedSubsetRole(
				'imgWithEmptyAlt',
				role
			);
			if (node.alt) {
		...
		input: (node, role) => {
			switch (node.type) {
				case 'button':
					return validateAriaAllowedSubsetRole('inputTypeButton', role);
				case 'checkbox':
					if (role === 'button' && node.hasAttribute('aria-pressed')) {
						return true;
					}
					return validateAriaAllowedSubsetRole('inputTypeCheckbox', role);
				case 'image':
					return validateAriaAllowedSubsetRole('inputTypeImage', role);
				case 'radio':
		...
		li: (node, role) => {
			...
			if (hasImplicitListitemRole) {
				return validateAriaAllowedSubsetRole('li', role);
			}
			return true;
		},
		link: node => !node.href,
		...
		svg: (node, role) => {
			...
			return validateAriaAllowedSubsetRole('svg', role);
		}
	};

	const element =
		axe.commons.aria.lookupTable.elementAriaAllowedRolesMap[tagName];
