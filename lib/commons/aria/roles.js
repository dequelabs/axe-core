...
aria.getRoleType = function(role) {
	const r = aria.lookupTable.role[role];
...
aria.implicitRole = function(node) {
	...
	const sortRolesByOptimalAriaContext = function(roles, ariaAttributes) {
		const getScore = function(role) {
			const allowedAriaAttributes = aria.allowedAttr(role);
	 ...
	 */
	const roles = Object.keys(aria.lookupTable.role).map(function(role) {
		const lookup = aria.lookupTable.role[role];
