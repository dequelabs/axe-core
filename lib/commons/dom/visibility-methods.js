...
export function overflowHidden(vNode, { isAncestor } = {}) {
  ...
  return nodes.some(node => {
    // a scrollable (overflow: auto/scroll) container between the node and the
    // overflow:hidden ancestor means the node can be scrolled back into view,
    // so the ancestor does not hide it
    if (hasScrollableAncestorBetween(vNode, node)) {
      return false;
    }
...
/**
 * Determine if a scrollable (overflow: auto/scroll) container sits between a
 * child and one of its ancestors (exclusive of both).
 * @param {VirtualNode} child
 * @param {VirtualNode} ancestor
 * @return {Boolean}
 */
function hasScrollableAncestorBetween(child, ancestor) {
  const scrollValues = ['auto', 'scroll', 'overlay'];
  let node = child.parent;
  while (node && node !== ancestor) {
    if (
      scrollValues.includes(node.getComputedStylePropertyValue('overflow-x')) ||
      scrollValues.includes(node.getComputedStylePropertyValue('overflow-y'))
    ) {
      return true;
    }
