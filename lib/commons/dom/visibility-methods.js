...
export function ariaHidden(vNode) {
  return getAriaValue(vNode, 'aria-hidden').value === 'true';
}
...
export function overflowHidden(vNode, { isAncestor } = {}) {
  ...
  return nodes.some(node => {
    ...
    // a scroll container between the node and the overflow:hidden ancestor
    // means the node can be scrolled back into view, so the ancestor does not
    // hide it
    if (isScrollableIntoView(vNode, node, rect, nodeRect, position)) {
      return false;
    }
...
/**
 * Determine if a scroll container between a node and its overflow:hidden
 * ancestor (exclusive of both) can bring that node back into view.
 *
 * A container only counts when it can scroll on the axis the node is outside
 * the ancestor on, when it has content to scroll there, and when it is itself
 * visible inside the ancestor - scrolling a container the ancestor already
 * clips away moves nothing into view.
 *
 * @param {VirtualNode} vNode
 * @param {VirtualNode} ancestor overflow:hidden ancestor
 * @param {DOMRect} rect vNode's bounding rect
 * @param {DOMRect} ancestorRect ancestor's bounding rect
 * @param {String} position vNode's computed position
 * @return {Boolean}
 */
function isScrollableIntoView(vNode, ancestor, rect, ancestorRect, position) {
  // an absolutely positioned node moves with a scroll container only when that
  // container holds the node's containing block
  const offsetParent =
    position === 'absolute' ? vNode.actualNode?.offsetParent : null;

  // the node has to recover on whichever axis the ancestor cuts it off
  const needsScrollX =
  ...
  while (node && node !== ancestor) {
    const elm = node.actualNode;
    // without a real node there is nothing to measure, so treat the ancestor
    // as hiding the node
    if (!elm) {
      return false;
    }

    const scrollsX = isScrollableOn(node, elm, 'x');
    const scrollsY = isScrollableOn(node, elm, 'y');
    if (
      (scrollsX || scrollsY) &&
      (!needsScrollX || scrollsX) &&
      (!needsScrollY || scrollsY) &&
      rectsOverlap(node.boundingClientRect, ancestorRect) &&
      (!offsetParent || elm.contains(offsetParent))
    ) {
      return true;
    }
...
/**
 * Determine if an element scrolls on an axis, and has content to scroll there.
 * @param {VirtualNode} vNode
 * @param {Element} elm
 * @param {String} axis 'x' or 'y'
 * @return {Boolean}
 */
function isScrollableOn(vNode, elm, axis) {
  const overflow = vNode.getComputedStylePropertyValue(`overflow-${axis}`);
