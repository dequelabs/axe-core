...
/**
 * Determine if a `button` is hidden because it is a descendant of a `select`.
 * Browsers never render a `button` inside a `select` and assistive technology
 * cannot reach it, no matter how deeply it is nested.
 * @see https://html.spec.whatwg.org/multipage/form-elements.html#the-select-element
 * @param {VirtualNode} vNode
 * @return {Boolean}
 */
export function selectButtonHidden(vNode) {
  if (vNode.props.nodeName !== 'button') {
    return false;
  }

  let node = vNode.parent;
  while (node) {
    if (node.props.nodeName === 'select') {
      return true;
    }
