...
const hiddenVNodeMethods = [nativelyHidden, detailsHidden];
const hiddenActualNodeMethods = [
  displayHidden,
  visibilityHidden,
  contentVisibiltyHidden
];
...
const isHiddenSelf = memoize(function isHiddenSelfMemoized(vNode, isAncestor) {
  if (hiddenVNodeMethods.some(method => method(vNode, { isAncestor }))) {
    return true;
  }
  ...
  if (hiddenActualNodeMethods.some(method => method(vNode, { isAncestor }))) {
    return true;
  }
