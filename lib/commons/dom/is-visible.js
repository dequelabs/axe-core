...
/**
 * Check if a `<details>` element is visible.
 * @param {HTMLDetailsElement} el The element to check
 * @retruns {boolean}
 */
function isDetailsVisible(el) {
  // IE11 does not support details (without a
  // polyfill), so we need to check for the `open`
  // property in the element to determine if
  // the browser supports hiding content when
  // the details element is closed
  return 'open' in el && el.hasAttribute('open');
}
