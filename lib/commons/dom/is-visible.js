...
export default function isVisible(el, screenReader, recursed) {
  ...
  const { DOCUMENT_NODE } = window.Node ?? {};

  // if we don't have a virtual node that means we've
  // recursed up the parent tree to a node that is
  // outside of the context. we still need to know
  // if the node and parents are hidden so need to
  // work with both virtual and non-virtual nodes
  ...
  const parent = el
    ? getComposedParent(el)
    : vNode.parent;
  let visible;
  if (parent) {
    visible = isVisible(parent, screenReader, true);
  } else {
    // if there is a non-virtual node that means
    // we're in a disconnected tree
