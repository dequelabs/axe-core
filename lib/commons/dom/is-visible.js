...
function isAreaVisible(el) {
	const rootEl = dom.getRootNode(el);
	/**
	 * Note:
	 * verified that `map` element cannot refer to `area` elements across different document trees
	 * hence the usage of `closest` as against `findUpVirtual`
	 */
	const mapEl = el.closest('map');
	if (!mapEl) {
		return false;
	}

	const mapElStyle = window.getComputedStyle(mapEl, null);
	if (mapElStyle.getPropertyValue('display') === 'none') {
		return false;
	}
	...
	const refs = rootEl.querySelectorAll(
