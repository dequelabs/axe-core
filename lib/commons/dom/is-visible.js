...
function isClipped(clip) {
	...
	const matchesClip = clip.match(clipRegex);
	const matchesClipPath = clip.match(clipPathRegex);
...
dom.isVisible = function(el, screenReader, recursed) {
	...
	if (
		...
		(!screenReader &&
			(isClipped(style.getPropertyValue('clip')) ||
				isClipped(style.getPropertyValue('clip-path')))) ||
		(!recursed &&
