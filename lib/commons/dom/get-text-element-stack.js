...
function getTextElementStack(node) {
  ...
  Array.from(node.childNodes).forEach(elm => {
    if (elm.nodeType === 3 && sanitize(elm.textContent) !== '') {
      ...
      // the parent rect so we stay without the bounds of the element
      ...
      const outsideRectBounds = Array.from(rects).some(rect => {
        return (
          ...
          rect.left + rect.width > nodeRect.left + nodeRect.width ||
          rect.top < nodeRect.top ||
          rect.top + rect.height > nodeRect.top + nodeRect.height
        );
      });
