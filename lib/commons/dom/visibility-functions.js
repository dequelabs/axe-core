...
export function visibilityHidden(vNode, recursed) {
  // Because the parent can be hidden, and the child visible we have to ignore visibility on ancestors
  // We don't need to look at the parent either, because visibility inherits.
 ...
 * @return {Boolean}
 */
export function scrollClipped(vNode) {
...
export function clipHidden(vNode) {
  ...
  const matchesClipPath = vNode.getComputedStylePropertyValue('clip-path')
    .match(clipPathRegex);
 ...
 * @retruns {Boolean}
 */
export function isAreaVisible(vNode, visibleFunction) {
  /**
   * Note:
