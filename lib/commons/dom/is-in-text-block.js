 ...
 * @property {Bool} returnLengths Return the text lengths used for the
 * comparison instead of a boolean
 * @return {Boolean|Object} Whether the element is in a text block, or the
 * parent and widget text lengths when `returnLengths` is true
 */
function isInTextBlock(
  node,
  { noLengthCompare, includeInlineBlock = false, returnLengths = false } = {}
) {
  ...
  if (isBlock(domNode) || (!includeInlineBlock && isInlineBlockLike(vNode))) {
    ...
    return returnLengths ? { parentTextLength: 0, widgetTextLength: 0 } : false;
  }
  ...
  if (noLengthCompare && !returnLengths) {
