...
dom.getTabbableElements = function getTabbableElements(virtualNode) {
	...
	const tabbableElements = nodeAndDescendents.filter(({ actualNode: el }) => {
		const elIsFocusable = dom.isFocusable(el);

		let tabIndex = el.getAttribute('tabindex');
		tabIndex =
			tabIndex && !isNaN(parseInt(tabIndex, 10)) ? parseInt(tabIndex) : null;

		return tabIndex ? elIsFocusable && tabIndex >= 0 : elIsFocusable;
	});
