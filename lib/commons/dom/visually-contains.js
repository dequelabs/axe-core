...
dom.visuallyContains = function visuallyContains(node, parent) {
	let nodeScrollParent = node;
	const parentScrollParent = getScrollParent(parent);
	...
	// scroll parent in between the the node and the parent since if the
	// element is off screen due to the scroll, it won't be visually contained
	// by the parent
	do {
		const nextScrollParent = getScrollParent(nodeScrollParent);

		if (
			nextScrollParent === parentScrollParent ||
			nextScrollParent === parent
		) {
			return contains(nodeScrollParent, parent);
		}

		nodeScrollParent = nextScrollParent;
	} while (nodeScrollParent);
