...
import { getNodeFromTree, getScroll, isShadowRoot, performanceTimer } from '../../core/utils';
...
let setupStart;
let nodeSetupStart;
let createTreeWalkerStart;
let createStackingOrderStart;
let findScrollRegionParentStart;
let getScrollStart;
let isVisibleOnScreenStart;
let addNodeToGridStart;

let setupTime = 0;
let nodeSetupTime = 0;
let createTreeWalkerTime = 0;
let createStackingOrderTime = 0;
let findScrollRegionParentTime = 0;
let getScrollTime = 0;
let isVisibleOnScreenTime = 0;
let addNodeToGridTime = 0;
...
export default function createGrid(
  ...
  setupStart = window.performance.now();
  ...
  if (root === document.body) {
    performanceTimer.mark('createGridStart');
  }
  ...
  setupTime += window.performance.now() - setupStart;

  // IE11 requires the first 3 parameters
  // @see https://developer.mozilla.org/en-US/docs/Web/API/Document/createTreeWalker
  createTreeWalkerStart = window.performance.now();
  ...
  createTreeWalkerTime += window.performance.now() - createTreeWalkerStart;
  let node = parentVNode ? treeWalker.nextNode() : treeWalker.currentNode;
  while (node) {
    nodeSetupStart = window.performance.now();
    ...
    nodeSetupTime += window.performance.now() - nodeSetupStart;

    createStackingOrderStart = window.performance.now();
    vNode._stackingOrder = createStackingOrder(vNode, parentVNode, nodeIndex++);
    createStackingOrderTime += window.performance.now() - createStackingOrderStart;

    findScrollRegionParentStart = window.performance.now();
    const scrollRegionParent = findScrollRegionParent(parentVNode);
    findScrollRegionParentTime += window.performance.now() - findScrollRegionParentStart;
    const grid = scrollRegionParent ? scrollRegionParent._subGrid : rootGrid;

    getScrollStart = window.performance.now();
    if (getScroll(vNode.actualNode)) {
      const subGrid = new Grid(vNode);
      vNode._subGrid = subGrid;
    }
    getScrollTime += window.performance.now() - getScrollStart;
    ...
    if (rect.width !== 0 && rect.height !== 0 && isVisibleOnScreen(node)) {
      ...
      addNodeToGridStart = window.performance.now();
      addNodeToGrid(grid, vNode);
      addNodeToGridTime += window.performance.now() - addNodeToGridStart;
    }
  ...
  if (root === document.body) {
    performanceTimer.mark('createGridEnd');
...
createGrid._logTime = function() {
  performanceTimer._log('Measure createGrid_setup took ' + setupTime + 'ms');
  performanceTimer._log('Measure createGrid_createTreeWalker took ' + createTreeWalkerTime + 'ms');
  performanceTimer._log('Measure createGrid_nodeSetup took ' + nodeSetupTime + 'ms');
  performanceTimer._log('Measure createGrid_createStackingOrder took ' + createStackingOrderTime + 'ms');
  performanceTimer._log('Measure createGrid_createStackingOrder_slice took ' + sliceTime + 'ms');
  performanceTimer._log('Measure createGrid_createStackingOrder_isStackingContext took ' + isStackingContextTime + 'ms');
  performanceTimer._log('Measure createGrid_createStackingOrder_getStackLevel took ' + getStackLevelTime + 'ms');
  performanceTimer._log('Measure createGrid_findScrollRegionParent took ' + findScrollRegionParentTime + 'ms');
  performanceTimer._log('Measure createGrid_getScroll took ' + getScrollTime + 'ms');
  performanceTimer._log('Measure createGrid_isVisibleOnScreenTime took ' + isVisibleOnScreenTime + 'ms');
  performanceTimer._log('Measure createGrid_addNodeToGrid took ' + addNodeToGridTime + 'ms');
}
 ...
 * @return {Boolean}
 */
const isStackingContext = memoize(function isStackingContextMemoized(vNode, parentVNode) {
...
let sliceStart;
let isStackingContextStart;
let getStackLevelStart;

let sliceTime = 0;
let isStackingContextTime = 0;
let getStackLevelTime = 0;
...
function createStackingOrder(vNode, parentVNode, treeOrder) {
  ...
  sliceTime += window.performance.now() - sliceStart;
  ...
  isStackingContextStart = window.performance.now();
  const stackingContext = isStackingContext(vNode, parentVNode)
  isStackingContextTime += window.performance.now() - isStackingContextStart;
  ...
  getStackLevelStart = window.performance.now();
  const stackLevel = getStackLevel(vNode, parentVNode);
  getStackLevelTime += window.performance.now() - getStackLevelStart;
  if (stackLevel !== null) {
 ...
 */
const findScrollRegionParent = memoize(function findScrollRegionParentMemoized(parentVNode) {
  ...
  if (parentVNode) {
    return findScrollRegionParent(parentVNode)
  }
