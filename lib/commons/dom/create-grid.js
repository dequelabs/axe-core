...
const FLOAT_ORDER = 0.25;
const STATIC_POSITION_ORDER = 0.5;
...
export default function createGrid(
  ...
  while (node) {
    ...
    vNode._stackingOrder = createStackingOrder(vNode, parentVNode);
...
function createStackingOrder(vNode, parentVNode) {
  ...
  if (!isStackingContext(vNode, parentVNode)) {
    if (vNode.getComputedStylePropertyValue('position') !== 'static') {
      ...
      stackingOrder.push(STATIC_POSITION_ORDER);
  ...
  const index = stackingOrder.findIndex(value =>
    [0, FLOAT_ORDER, STATIC_POSITION_ORDER].includes(value)
  );
  if (index !== -1) {
