...
function addNodeToGrid(grid, vNode) {
  ...
  vNode.clientRects.forEach(rect => {
    ...
    // an element that ends at a grid line should
    // not be added to the next grid cell
    const endRow = Math.floor((y + rect.height - 1) / gridSize);
    const endCol = Math.floor((x + rect.width - 1) / gridSize);

    grid.numCols = Math.max(grid.numCols ?? 0, endCol);

    addToGrid(grid, { startRow, endRow, startCol, endCol }, vNode);
  });
}

function addToGrid(grid, { startRow, endRow, startCol, endCol }, vNode) {
  grid.cells._negativeIndex ??= 0;
  loopGridCells(grid.cells, startRow, endRow, (_, row) => {
    grid.cells[row] = grid.cells[row] || [];
    grid.cells[row]._negativeIndex ??= 0;

    loopGridCells(grid.cells[row], startCol, endCol, (_, col) => {
...
function loopGridCells(cells, start, end, callback) {
  if (start < cells._negativeIndex) {
    let num = cells._negativeIndex - start;
    cells._negativeIndex = start;
    while (num--) {
      cells.splice(0, 0, []);
    }
  }

  start -= cells._negativeIndex;
  end -= cells._negativeIndex;

  for (let index = start; index <= end; index++) {
    callback(cells[index], index, index + cells._negativeIndex);
  }
}
