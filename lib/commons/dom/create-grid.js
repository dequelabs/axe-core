...
function addNodeToGrid(grid, vNode) {
  vNode.clientRects.forEach(rect => {
    ...
    vNode._grid ??= grid;
    const gridRect = grid.getPositionOfRect(rect);
...
class Grid {
  ...
  loopGridPosition(gridRect, callback) {
    const { left, right, top, bottom } = gridRect;
    if (this.boundaries) {
      gridRect = getBoundingRect(this.boundaries, gridRect);
    }
    this.boundaries = gridRect;

    loopNegativeIndexMatrix(this.cells, top, bottom, gridRow => {
      loopNegativeIndexMatrix(gridRow, left, right, callback);
    });
  }

  getPositionOfRect({ top, right, bottom, left }, margin = 0) {
