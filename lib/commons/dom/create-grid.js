...
function addNodeToGrid(grid, vNode) {
  vNode.clientRects.forEach(rect => {
    ...
    vNode._grid ??= grid;
    const gridRect = getGridPositionOfRect(rect);
    grid.boundaries = grid.boundaries
      ? getBoundingRect(grid.boundaries, gridRect)
      : gridRect;

    loopGridPosition(grid, gridRect, gridCell => {
...
export function loopGridPosition(grid, gridRect, callback) {
  const { left, right, top, bottom } = gridRect;
  loopNegativeIndexMatrix(grid.cells, top, bottom, gridRow => {
    loopNegativeIndexMatrix(gridRow, left, right, callback);
  });
}
...
export function getGridPositionOfRect(
  ...
  top = toGridIndex(top - margin);
  right = toGridIndex(right + margin - 1);
  bottom = toGridIndex(bottom + margin - 1);
  left = toGridIndex(left - margin);
  return new window.DOMRect(left, top, right - left, bottom - top);
}

export function getGridCellFromPoint({ cells, boundaries }, { x, y }, debug) {
  const rowIndex = toGridIndex(y);
  const colIndex = toGridIndex(x);
  const { top, right, bottom, left } = boundaries;

  if (debug) {
    console.log({ x, y }, { rowIndex, colIndex }, { top, right, bottom, left });
  }
  if (!isPointInRect({ y: rowIndex, x: colIndex }, boundaries)) {
