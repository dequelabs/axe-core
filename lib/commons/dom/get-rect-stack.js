...
export function getRectStack(grid, rect, recursed = false) {
  ...
  const x = toFixed(rect.left + rect.width / 2);
  const y = toFixed(rect.top + rect.height / 2);
  ...
  let stack =
    grid.cells[row][col]?.filter(gridCellNode => {
      return gridCellNode.clientRects.find(clientRect => {
        ...
        return (
          x < toFixed(rectX + clientRect.width) &&
          x >= toFixed(rectX) &&
          y < toFixed(rectY + clientRect.height) &&
          y >= toFixed(rectY)
        );
      });
    }) ?? [];
...
/**
 * Browsers handle floating point precision of bounding rects differently, so we need to normalize the size to account for this.
 */
