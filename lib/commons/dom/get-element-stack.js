...
function findScrollRegionParent(vNode, vNodeParent) {
	...
	while (vNodeParent) {
		...
		vNodeParent = axe.utils.getNodeFromTree(
			vNodeParent.actualNode.parentElement
		);
	}
...
function getRectStack(grid, rect, recursed = false) {
	...
	let stack = grid.cells[row][col].filter(gridCellNode => {
		if (window.debugThisTest) {
			console.log(
				'gridCellNode:',
				gridCellNode.actualNode.cloneNode().outerHTML
			);
		}
	...
	if (window.debugThisTest) {
		console.log(
			'stack:',
			stack.map(vNode => vNode.actualNode.cloneNode().outerHTML)
		);
	}
...
dom.getElementStack = function(node) {
	...
	if (window.debugThisTest) {
		console.log(
			'grid[0][0]:',
			grid.cells[0][0].map(vNode => vNode.actualNode.cloneNode().outerHTML)
		);
	}
