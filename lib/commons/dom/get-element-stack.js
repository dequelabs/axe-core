 ...
 * @return {Boolean}
 */
function isStackingContext(vNode, vNodeParent) {
	...
	if (zIndex !== 'auto' && vNodeParent) {
		const parentDsiplay = vNodeParent.getComputedStylePropertyValue('display');
		if (
			[
				'flex',
				'inline-flex',
 ...
 * @return {Number[]}
 */
function getStackingOrder(vNode, vNodeParent) {
	const stackingOrder = vNodeParent._stackingOrder.slice();
	const zIndex = vNode.getComputedStylePropertyValue('z-index');
	if (zIndex !== 'auto') {
		stackingOrder[stackingOrder.length - 1] = parseInt(zIndex);
	}
	if (isStackingContext(vNode, vNodeParent)) {
 ...
 */
function findScrollRegionParent(vNode, vNodeParent) {
	...
	while (vNodeParent) {
		...
		vNodeParent = axe.utils.getNodeFromTree(
			vNodeParent.actualNode.parentElement || vNodeParent.actualNode.parentNode
		);
	}
...
function createGrid(
	...
	while (node) {
		...
		// an svg in IE11 has not parentElement but instead has a parentNode
		const vNodeParent = node.parentElement
			? axe.utils.getNodeFromTree(node.parentElement)
			: // parentNode could be a shadow root so we need to verity it's
			// in the tree first
			node.parentNode && axe.utils.getNodeFromTree(node.parentNode)
			? axe.utils.getNodeFromTree(node.parentNode)
			: parentVNode;

		if (!vNode) {
			vNode = new VirtualNode(node, vNodeParent);
		}

		vNode._stackingOrder = getStackingOrder(vNode, vNodeParent);

		const scrollRegionParent = findScrollRegionParent(vNode, vNodeParent);
