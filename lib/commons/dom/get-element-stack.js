...
function isStackingContext(vNode) {
	...
	if (
		...
		if (
			parent.getComputedStylePropertyValue('display') === 'flex' ||
			parent.getComputedStylePropertyValue('display') === 'inline-flex'
		) {
			return true;
		}
	}
...
function isScrollRegion(vNode) {
	return (
		vNode.getComputedStylePropertyValue('overflow-x') === 'auto' ||
		vNode.getComputedStylePropertyValue('overflow-x') === 'scroll' ||
		vNode.getComputedStylePropertyValue('overflow-y') === 'auto' ||
		vNode.getComputedStylePropertyValue('overflow-y') === 'scroll'
	);
}
		...
		return rects.find(rect => {
			...
			if (
				...
				(replacedElements.includes(nodeName) ||
					(vNode.getComputedStylePropertyValue('display') !== 'inline' &&
						vNode.getComputedStylePropertyValue('display') !== 'contents'))
			) {
