...
function isModalOpen(options) {
  ...
  return cache.get('isModalOpen', detectModal(modalPercent));
}

function detectModal(modalPercent) {
  ...
  if (definitelyModalExists()) {
    return true;
  } else if (couldBeModalExists(modalPercent)) {
    return true;
  }
  return undefined;
}

function definitelyModalExists() {
  const definiteModalRoles = 'dialog, [role=dialog], [aria-modal=true]';
  return Boolean(
...
function couldBeModalExists(modalPercent) {
  ...
  for (let i = 0; i < stacks.length; i++) {
    ...
    const modalElement = firstModalInStack(
      stacks[i],
      percentWidth,
      percentHeight
    );
...
function firstModalInStack(stack, percentWidth, percentHeight) {
  return stack.find(elm => {
