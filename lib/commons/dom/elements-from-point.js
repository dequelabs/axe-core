...
/**
 * is true if the browser supports one of the methods to get elements from point
 */
dom.pointerEventSupport = (function () {
	var element = document.createElement('x');
	element.style.cssText = 'pointer-events:auto';
	return element.style.pointerEvents === 'auto' || !!document.msElementsFromPoint;
}());
...
dom.elementsFromPoint = function (doc, x, y) {
	var elements = [], previousPointerEvents = [], current, i, d, cssProp, cssDisableVal;
	if (dom.pointerEventSupport) {
		cssProp = 'pointer-events';
		cssDisableVal = 'none';
	} else {
		cssProp = 'visibility';
		cssDisableVal = 'hidden';
	}
