 ...
 *
 * ```js
 * match.fromPrimitive('foo', 'foo') // true, string is the same
 * match.fromPrimitive('foo', ['foo', 'bar']) // true, string is included
 * match.fromPrimitive('foo', /foo/) // true, string matches regex
 * match.fromPrimitive('foo', str => str.toUpperCase() === 'FOO') // true, function return is truthy
 * match.fromPrimitive('foo', '/foo/') // true, string matches regex string
 * ```
 *
 * @private
 ...
 * @returns {Boolean}
 */
export default function fromPrimitive(someString, matcher) {
