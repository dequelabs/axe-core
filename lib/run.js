 ...
 */
function collectResultsFromFrames(context, options, command, parameter, callback) {
	'use strict';
 ...
 */
function runAnalysis(context, options, callback) {
	'use strict';
	...
	if (context.frames.length) {
		q.defer(function (done) {
			collectResultsFromFrames(context, options, 'analyze', null, done);
		});
	}
	q.defer(function (cb) {
...
function runClassifier(classifierId, context, options, callback) {
	...
	if (context.frames.length) {
		q.defer(function (done) {
			collectResultsFromFrames(context, options, 'classify', classifierId, done);
		});
	}
	...
	q.then(function (data) {
		// Add wrapper object so that we may use the same "merge" function for results from inside and outside frames
		callback(mergeResults(data.map(function (d) {
			return {
