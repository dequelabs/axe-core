...
Rule.prototype.run = function run(context, options = {}, resolve, reject) {
  ...
  if (this.id === 'color-contrast') {
    window.performance.mark('color-contrast-checks-start');
  }
  ...
  q.then(() => {
    ...
    if (this.id === 'color-contrast') {
      ['any', 'all', 'none'].forEach(type => {
        this[type].forEach(c => {
          const check = this._audit.checks[c.id || c];
          if (check.evaluate._logTime) {
            check.evaluate._logTime();
          }
        });
      });

      window.performance.mark('color-contrast-checks-end');
      window.performance.measure('color-contrast-checks', 'color-contrast-checks-start', 'color-contrast-checks-end');

    }

    if (options.performanceTimer) {
      this._logRulePerformance();
      // cache.get('ruleTimeout', () => window.performance.now());
    }
    ...
    setTimeout(() => {
      // if (options.performanceTimer) {
      //   const timeoutStart = cache.get('ruleTimeout');
      //   const timeoutEnd = window.performance.now();
      //   const diff = timeoutEnd - timeoutStart;
      //   performanceTimer._log(`Measure timeout_${this.id} took ${diff}ms`);
      //   cache.set('ruleTimeout', timeoutEnd);
      // }
      resolve(ruleResult);
    }, 0);
  }).catch(error => {
...
Rule.prototype.gatherAndMatchNodes = function gatherAndMatchNodes(
  ...
  if (this.id === 'color-contrast') {
    window.performance.mark('color-contrast-gather-start');
  }
  ...
  if (this.id === 'color-contrast') {
    this.matches._logTime();
    window.performance.mark('color-contrast-gather-end');
    window.performance.measure('color-contrast-gather', 'color-contrast-gather-start', 'color-contrast-gather-end');
  }
