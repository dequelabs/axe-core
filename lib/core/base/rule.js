...
Rule.prototype.gather = function(context) {
	'use strict';
	const elements = axe.utils.select(this.selector, context);
...
Rule.prototype.runChecks = function(type, node, options, resolve, reject) {
	...
	const self = this;
	const checkQueue = axe.utils.queue();
...
function findCheckResults(nodes, checkID) {
	...
	nodes.forEach(function(nodeResult) {
		const checks = axe.utils.getAllChecks(nodeResult);
...
function sanitizeNodes(result) {
	...
	let nodes = result.nodes.filter(function(detail) {
		let length = 0;
...
Rule.prototype.after = function(result, options) {
	...
	afterChecks.forEach(function(check) {
		...
		const option = axe.utils.getCheckOption(check, ruleID, options);

		const afterResults = check.after(beforeResults, option);
