...
function normaliseProps(serialNode) {
  ...
  const keys = Object.keys(nodeNamesToTypes);
  Object.values(nodeNamesToTypes).forEach((value, index) => {
    nodeTypeToName[value] = keys[index];
  });

  const nodeName = serialNode.nodeName ?? nodeTypeToName[serialNode.nodeType];
  const nodeType =
  ...
  assert(
    nodeNamesToTypes[nodeName] === nodeType ||
      !(nodeName in nodeNamesToTypes) ||
