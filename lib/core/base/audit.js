...
export default class Audit {
  ...
  _init() {
    ...
    this.selectorConfig = { ignoredAttributes: [] };
  ...
  run(context, options, resolve, reject) {
    ...
    if (this.selectorConfig) {
      if (typeof options.selectors !== 'object') {
        if (options.selectors !== false) {
          options.selectors = { ...this.selectorConfig };
        }
      } else {
        options.selectors = { ...this.selectorConfig, ...options.selectors };
        if (
          this.selectorConfig.ignoredAttributes &&
          options.selectors.ignoredAttributes
        ) {
          options.selectors.ignoredAttributes = [
            ...new Set([
              ...this.selectorConfig.ignoredAttributes,
