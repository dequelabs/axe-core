...
class Audit {
  ...
  /**
   * Set the runOnly option
   * @throws {Error} If any tag or rule specified in options is unknown
   * @param {Mixed} Array or object of tags or rule names that should be run
   */
  setRunOnly(runOnly) {
    const ruleIds = [];
    const tags = [];
    ...
    // Validate runOnly
    if (runOnly && typeof runOnly === 'object') {
      if (Array.isArray(runOnly)) {
        const hasTag = runOnly.find(value => tags.includes(value));
        const hasRule = runOnly.find(value => ruleIds.includes(value));
        if (hasTag && hasRule) {
      ...
      // Check if every value in runOnly is a known rule ID
      if (['rule', 'rules'].includes(only.type)) {
        only.type = 'rule';
        only.values.forEach(ruleId => {
          if (!ruleIds.includes(ruleId)) {
            throw new Error('unknown rule `' + ruleId + '` in runOnly');
          }
        });
  ...
  normalizeOptions(options) {
    ...
    this.setRunOnly(options.runOnly);
    options.runOnly = this.runOnly;
