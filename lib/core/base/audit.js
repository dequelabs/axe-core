...
const performanceMarker = (id, shouldMark) => {
	if (shouldMark) {
		const marker = {
...
Audit.prototype.run = function (context, options, resolve, reject) {
	...
	axe.utils.preload(options)
		.then(([preloadedAssets]) => { // q - retuns array -> destructure/ pluck the first item, is an key value map of preloadedAssets
			// mutate options with preloadedAssets
			options = {
				...options,
				preloadedAssets
			};
			// deduce a queue of rules to execute, extended with preloadedAssets passed to checks
			...
			this.rules
				.forEach((rule) => {
					if (axe.utils.ruleShouldRun(rule, context, options)) {
						const marker = performanceMarker(rule.id, options.performanceTimer);
						q.defer((resolve, reject) => {
							rule.run(context, options,
								(result) => {
									marker();
									resolve(result);
								},
