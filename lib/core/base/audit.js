 ...
 */
function getDefferedRule(rule, context, options, assets) {
...
Audit.prototype.run = function(context, options, resolve, reject) {
	...
	runNowRules.forEach(rule => {
		...
		nowQueue.defer(deferredRule);
	});

	// defer preload into the NOW queue
	// only if there are rules that need assets
	if (runLaterRules.length) {
		const preloader = axe.utils.preload(options);
		nowQueue.defer(preloader);
	}

	// invoke the now queue
	nowQueue
		.then(nowQueueResults => {
			// if there are no rules to run LATER - resolve with rule results
			if (!runLaterRules.length) {
				resolve(nowQueueResults.filter(result => !!result));
			}

			// if there are rules to be run LATER based on assets
			// interpolate results into separate variables
			// as preloader was the last deferred,
			// the last item in the results is the preloaded assets (hence pop())
			// extract first item in array as `queue` returns an array
			const preloadedAssets = nowQueueResults.pop()[0];

			// the reminder of the results are RuleResults
			const nowRulesResults = nowQueueResults;

			// init a LATER queue for rules that are dependant on preloaded assets
			const laterQueue = axe.utils.queue();
			runLaterRules.forEach(rule => {
				const deferredRule = getDefferedRule(
					rule,
					context,
					options,
					preloadedAssets
				);
				laterQueue.defer(deferredRule);
			});
			...
			laterQueue
				.then(laterRuleResults => {
					...
					// concat results
					const ruleResults = nowRulesResults.concat(laterRuleResults);

					// resolve
					resolve(ruleResults.filter(result => !!result));
				})
				.catch(reject);
		})
		.catch(reject);
