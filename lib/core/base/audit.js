...
const measurePerformance = (id) => {
	const marker = {
		start: `mark_rule_start_${id}`,
		end: `mark_rule_end_${id}`
	}
	axe.utils.performanceTimer.mark(marker.start);
	return () => {
		axe.utils.performanceTimer.mark(marker.end);
		axe.utils.performanceTimer.measure(`rule_${id}`, marker.start, marker.end);
	};
};
...
const getRulesQ = ({ rules, conditionFn, extend }) => {
	return rules
		.reduce((out, r) => {
			if (conditionFn(r)) {
...
Audit.prototype.run = function (context, options, resolve, reject) {
		...
		.reduce((out, rule) => {
			if (axe.utils.ruleShouldRun(rule, context, options)) {
				out.push({
					...
					context,
					options,
					performanceMeasurer: noop,
					// performanceMeasurer: options.performanceTimer
					// 	? measurePerformance(rule.id) : noop,
					preload: rule.preload.length > 0
				});
			}
			return out;
		}, []);
	...
	// TODO:JEY - do not use promise - try using q.
	Promise
		.all([
			// all rules which do not need preload
			getRulesQ({
				rules: toRunRules,
				conditionFn: (r) => !r.preload
			}),
			// start preloading
			axe.utils.preload(options)
		])
		.then((results) => {
			const axeResults = results[0];
			const preloadAssets = results[1];
			// run rules which need preloaded assets
