...
// TODO:JEY - doc
function getSplitRulesToRun(rules, context, options) {
	const base = {
		asap: [],
		postPreload: []
	};
	const out = rules.reduce((out, rule) => {
		if (!axe.utils.ruleShouldRun(rule, context, options)) {
			return out;
		}
		if (rule.preload) {
			out.postPreload.push(rule);
			return out;
		}
		out.asap.push(rule);
		return out;
...
// TODO:JEY - doc
function getDefferedRule(rule, context, options, preloadedAssets) {
	...
	if (preloadedAssets) {
		context.preloadedAssets = preloadedAssets;
	}

	const out = function(res, rej) {
		rule.run(
			context,
			options,
			function(out) {
...
Audit.prototype.run = function(context, options, resolve, reject) {
	...
	const toRunRules = getSplitRulesToRun(this.rules, context, options);
	const toRunAsapRulesQ = toRunRules.asap.reduce((out, rule) => {
		out.defer(getDefferedRule(rule, context, options));
		return out;
	}, axe.utils.queue());
	const preloadQ = axe.utils.queue(); // TODO:JEY - this will be replaced by method from #PR - 958 - axe.utils.preload(options)
	const preQ = axe.utils.queue();
	preQ.defer(toRunAsapRulesQ);
	preQ.defer(preloadQ);
	preQ
		.then(preQueueResults => {
			const preRunRuleResults = preQueueResults[0];
			const preloadedAssets = preQueueResults[1];
			const postQ = toRunRules.postPreload.reduce((out, rule) => {
				out.defer(getDefferedRule(rule, context, options, preloadedAssets));
				return out;
			}, axe.utils.queue());
			postQ
				.then(postQueueResults => {
					axe._selectCache = undefined; // remove the cache
					const ruleResults = [...preRunRuleResults, ...postQueueResults];
					resolve(
						ruleResults.filter(function(result) {
							return !!result;
						})
					);
				})
