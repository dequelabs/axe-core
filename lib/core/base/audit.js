	...
	if (runLaterRules.length) {
		preloaderQueue.defer((res, rej) => {
			...
			axe.utils
				.preload(options)
				.then(preloadResults => {
					// pluck only item in results (because it is a queue) and resolve
					const assets = preloadResults[0];
					res(assets);
				})
				.catch(err => {
					// resolve with empty assets to allow rule.run to continue
	...
	queueForNowRulesAndPreloader
		.then(nowRulesAndPreloaderResults => {
			...
			if (assetsFromQueue && assetsFromQueue.length) {
				// result is a queue, hence the index resolution
