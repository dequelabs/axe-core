...
Check.prototype.matches = function (node) {
	return (!this.selector || axe.utils.matchesSelector(node, this.selector));
};
...
Check.prototype.configure = function (spec) {
	...
	['evaluate', 'after', 'matches']
	.filter( prop => spec.hasOwnProperty(prop) )
	.forEach( prop => this[prop] = createExecutionContext(spec[prop]) );

	if (typeof spec.metadata === 'object') {
		// override old values
		this.metadata = Object.assign(this.metadata, spec.metadata);

		// Transform messages into functions:
		if (typeof this.metadata.messages === 'object') {
			Object.keys(this.metadata.messages)
			.filter( prop => spec.hasOwnProperty(prop) )
			.forEach( prop => this[prop] = createExecutionContext(spec[prop]) );
		}
	}
