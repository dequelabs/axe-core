...
Check.prototype.run = function (node, options, callback) {
	...
	if (enabled && this.matches(node)) {
		...
		q.defer(function (resolve) {
			var q = utils.queue();
			checkHelper.defer = q.defer;
			syncResult = self.evaluate.call(checkHelper, node, checkOptions);
			q.then(function (asyncResult) {
				if (asyncResult.length) {
					checkResult.value = asyncResult[0];
				} else {
					checkResult.value = syncResult;
				}
				resolve(checkResult);
			});
		});
