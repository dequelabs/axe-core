 ...
 */
export default function Context(spec, flatTree, options = {}) {
  spec = clone(spec);
  this.frames = [];
  ...
  if (options.performanceTimer) {
    performanceTimer.mark('getFlattenedTreeStart');
  }
  this.flatTree = flatTree ?? getFlattenedTree(getRootNode(spec));
  if (options.performanceTimer) {
    performanceTimer.mark('getFlattenedTreeEnd');
    performanceTimer.measure(
      'getFlattenedTree',
      'getFlattenedTreeStart',
      'getFlattenedTreeEnd'
    );
  }
