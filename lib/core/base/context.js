...
function pushUniqueFrame(collection, frame) {
	...
	const fr = axe.utils.findBy(collection, 'node', frame);
...
function pushUniqueFrameSelector(context, type, selectorArray) {
	...
	let result;
	const frames = document.querySelectorAll(selectorArray.shift());

	frameloop: for (let i = 0, l = frames.length; i < l; i++) {
		let frame = frames[i];
		for (let j = 0, l2 = context.frames.length; j < l2; j++) {
...
function validateContext(context) {
	...
	if (context.include.length === 0) {
		if (context.frames.length === 0) {
			const env = axe.utils.respondable.isInFrame() ? 'frame' : 'page';
...
function Context(spec) {
	...
	const err = validateContext(this);
