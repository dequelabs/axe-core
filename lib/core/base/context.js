...
function pushUniqueFrameSelector(context, type, selectorArray) {
  ...
  const frames = querySelectorAll(context.flatTree, frameSelector);
...
function parseSelectorArray(context, type) {
  ...
  for (var i = 0, l = context[type].length; i < l; i++) {
    ...
    if (typeof item === 'string') {
      nodeList = Array.from(querySelectorAll(context.flatTree, item));
      ...
      if (item.length > 1) {
        ...
        nodeList = Array.from(querySelectorAll(context.flatTree, item[0]));
        //eslint no-loop-func:0
        result = result.concat(
          nodeList.map(node => {
            return getNodeFromTree(node);
