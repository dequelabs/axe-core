...
export default function earlReporter(results, options, callback) {
  ...
  // getEnvironmentData passes a caller-supplied environmentData object through
  // verbatim, so url may be absent or a non-string; earlAssertion requires a
  // string source, so normalize anything non-string to an empty string.
  const reportedUrl = getEnvironmentData(environmentData).url;
  const source = typeof reportedUrl === 'string' ? reportedUrl : '';
