...
export function getNodesMatchingExpression(domTree, expressions, filter) {
  ...
  expressions.forEach(expression =>
    findMatchingNodes(expression, selectorMap, shadowId, nodeSet)
  );
 ...
 * @param {Set} nodeSet
 */
function findMatchingNodes(expression, selectorMap, shadowId, nodeSet) {
  ...
  if (isGlobalSelector(exp)) {
    ...
    if (exp.tag && exp.tag !== '*') {
      ...
      nodes = nodes.length ? getSharedValues(cachedNodes, nodes) : cachedNodes;
    }
    ...
    if (exp.attributes) {
      for (let i = 0; i < exp.attributes.length; i++) {
        ...
        nodes = nodes.length
          ? getSharedValues(cachedNodes, nodes)
          : cachedNodes;
      }
    }
  }
 ...
 * @param {Object} map
 */
function cacheSelector(key, vNode, map = selectors) {
  map[key] = map[key] || [];
  map[key].push(vNode);
}
...
export function cacheNodeSelectors(vNode) {
  ...
  vNode.attrNames.forEach(attrName => {
    ...
    if (attrName === 'id') {
      selectors[idsKey] = selectors[idsKey] || {};
      tokenList(vNode.attr(attrName)).forEach(value => {
        cacheSelector(value, vNode, selectors[idsKey]);
      });
    }
