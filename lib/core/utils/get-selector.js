...
export function getSelectorData(domTree) {
  ...
  while (currentLevel.length) {
    ...
    if (!!node.querySelectorAll) {
      ...
      if (node.id) {
        values[`#${node.id}`] = 1;
        newVals.push(`#${node.id}`)

        data.otherFreq[tag][`#${node.id}`] ??= new Set();
        data.otherFreq[tag][`#${node.id}`].add(node);
      }
      ...
      if (node.classList) {
        Array.from(node.classList).forEach(cl => {
          ...
          values[`.${ind}`] = 1
          newVals.push(`.${ind}`)
          data.otherFreq[tag][`.${ind}`] ??= new Set();
          data.otherFreq[tag][`.${ind}`].add(node);
          if (data.classes[ind]) {
            data.classes[ind]++;
          } else {
            data.classes[ind] = 1;
        ...
        Array.from(getNodeAttributes(node))
          ...
          .forEach(at => {
            ...
            if (atnv) {
              values[`[${atnv}]`] = 1;
              newVals.push(`[${atnv}]`)
              data.otherFreq[tag][`[${atnv}]`] ??= new Set();
              data.otherFreq[tag][`[${atnv}]`].add(node);
      ...
      data.frequency[tag] ??= new Map();
      data.frequency[tag].set(node, values);
      data.newFreq[tag] ??= new Map();
      data.newFreq[tag].set(node, newVals);
    }
...
function leastFrequent(node, freqList) {
  // shadow dom stuff causes problems
  if (!freqList) {
    return {
      selector: '',
      unique: false
...
// function leastFrequent(node, feqList) {
//   let selector = '';

//   // Insert all elements in hash.
//   const map = new Map(feqList);
//   const keys = JSON.parse(JSON.stringify(map.get(node)))

//   let min_count = Infinity;
//   while (min_count > 1) {
//     const count = new Map();

//     map.forEach((values, i) => {
//       let hasKey = 0;
//       const arr = Object.keys(values).forEach(key => {
//         if (!keys[key]) {
//           return;
//         }
...
//       return {
//         selector: getBaseSelector(node) + selector,
//         unique: map.size <= 1
//       };
//     }
//   }
...
function generateSelector(elm, options, doc) {
  ...
  // do {
    ...
    // const similar = findSimilar(doc, features);
    // if (similar.length === 1) {
    //   vNode._selector = features;
    //   return features;
    // }
    const { selector, unique } = leastFrequent(elm, axe._selectorData.newFreq[elm.nodeName]);
    ...
    if (unique) {
      ...
      // would probably need to add :nth-child to the end since selector is not guaranteed to be unique
      // to its siblings
      return generateSelector(elm.parentElement, options, doc) + ' > ' + selector;
    }

    // else if (similar.length > 1) {
    //   return generateSelector(elm.parentElement, options, doc) + ' > ' + features;
    // }

    // vNode._selector = ':root';
    // return ':root';
