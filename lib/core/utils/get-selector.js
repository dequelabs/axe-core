...
const INTERNAL_IGNORED_ATTRIBUTES = [
  'class',
  'style',
  'id',
  'selected',
 ...
 */
function filterAttributes(at, options = {}) {
  const ignoredAttributes = options.ignoredAttributes || [];
  return (
    !INTERNAL_IGNORED_ATTRIBUTES.includes(at.name) &&
 ...
 */
export function getSelectorData(domTree, options = {}) {
  ...
  const ignoredAttributes = (options && options.ignoredAttributes) || [];
  const skipClasses = ignoredAttributes.includes('class');
    ...
    if (!!node.querySelectorAll) {
      ...
      if (node.hasAttributes()) {
        Array.from(getNodeAttributes(node))
          .filter(at => filterAttributes(at, options))
          .forEach(at => {
 ...
 * @param {Object} selectorData	The statistics on feature frequency
 * @param {Object} options	 Options for how to generate the selector
 * @return {Array}						The sorted array of uncommon class data
 */
function uncommonClasses(node, selectorData, options = {}) {
  ...
  if (node.classList && !ignoredAttributes.includes('class')) {
    Array.from(node.classList).forEach(cl => {
      const ind = escapeSelector(cl);
 ...
 * Get ID selector
 */
function getElmId(elm, options = {}) {
  const ignoredAttributes = options.ignoredAttributes || [];
  if (ignoredAttributes.includes('id') || !elm.getAttribute('id')) {
    return;
  }
 ...
 */
function uncommonAttributes(node, selectorData, options = {}) {
  const retVal = [];
...
function getThreeLeastCommonFeatures(elm, selectorData, options = {}) {
  let selector = '';
  let features;
  const clss = uncommonClasses(elm, selectorData, options);
  const atts = uncommonAttributes(elm, selectorData, options);
...
function generateSelector(elm, options, doc) {
   ...
   */
  do {
    let features = getElmId(elm, options);
    if (!features) {
      features = getThreeLeastCommonFeatures(elm, axe._selectorData, options);
