...
function matchTags(rule, runOnly) {
	...
	var defaultExclude = (axe._audit.tagExclude || []);
	...
	if (runOnly.include || runOnly.exclude) {
		...
		exclude = exclude.concat(defaultExclude.filter((tag) => include.indexOf(tag) === -1));
	...
	// Otherwise, only use the include value, ignore exclude
		...
		exclude = defaultExclude.filter((tag) => include.indexOf(tag) === -1);
	}

	matching = include.some((tag) => rule.tags.indexOf(tag) !== -1);
	if (matching || (include.length === 0 && rule.enabled !== false)) {
		return exclude.every((tag) => rule.tags.indexOf(tag) === -1);
	} else {
		return false;
	}
}
