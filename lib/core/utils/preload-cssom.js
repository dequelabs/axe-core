...
axe.utils.preloadCssom = function preloadCssom({
	...
	/**
	 * initialize factory function with dynamicDoc, which returns a function to resolve CSSStylesheet from given data
	 */
	 ...
	 *
	 * @method getCssomForAllRootNodes
	 * @private
	 * @param {Object} resolve resole callback
	 * @param {Object} reject reject callback
	 */
	function getCssomForAllRootNodes(resolve, reject) {
		const rootNodesCssomQueue = axe.utils.queue();
		const rootNodesCssomQueueResults = rootNodes.reduce(
			getCssomForRootNode,
			rootNodesCssomQueue
		);

		rootNodesCssomQueueResults
			.then(assets => {
	...
	/**
	 * Reducer function, that operated on a given root object
	 *
	 * @method getCssomForRootNode
	 * @private
	 * @param {Object} cssomQueue `axe.utils.queue` to defer cssom processing fro given node
	 * @param {Object} param  Enhanced document or documentFragment object returned from `getAllRootNodesInTree`
	 * @param {Number} index given index of root
	 * @returns {Object} `axe.utils.queue`
	 */
	function getCssomForRootNode(cssomQueue, { rootNode, shadowId }, index) {
		cssomQueue.defer((resolve, reject) => {
			/**
			 * Load CSSOM assets for given root node
			 */
			const options = {
				rootNode,
				shadowId,
 ...
 * NOTE:
 * using `axe.utils.queue` from various `loadCssom` paths, returns a nested array of arrays
 * -> flatten arrays
 * -> filter `undefined` results
 *
 * @method processCssomAssets
 * @private
 ...
 *
 * @method parseNonCrossOriginStylesheet
 * @private
...
function parseNonCrossOriginStylesheet(sheet, options, priority) {
	...
	cssImportRules.forEach((importRule, cssRuleIndex) =>
		q.defer((resolve, reject) => {
			...
			axe.imports
				.axios(axiosOptions)
				.then(({ data }) => {
					const sheet = options.convertDataToStylesheet({
						data,
						isExternal: true,
	...
	q.defer(resolve =>
		resolve(
			options.convertDataToStylesheet({
				data: convertCssTextFromCssRulesToString(nonImportCSSRules),
				isExternal: false,
				priority,
				root: options.rootNode,
				shadowId: options.shadowId
	...
	/**
	 * Parse all CSSRules and interpret `cssText` and concatenate to string.
	 *
	 * @method convertCssTextFromCssRulesToString
	 * @private
	 * @param {Object} rules cssRules
	 * @returns {String}
	 */
	function convertCssTextFromCssRulesToString(rules) {
		return rules
			.reduce((out, rule) => {
 ...
 *
 * @method getStylesheetsFromDocumentFragment
 * @private
 * @param {Object} options configuration options of `loadCssom`
 ...
 *
 * @method getStylesheetsFromDocument
 * @private
 ...
 *
 * @method filerStyleAndLinkAttributesInDocumentFragment
 * @private
