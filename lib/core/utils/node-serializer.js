...
let cloneLimitedDqElementStart;
let toJSONStart;

let cloneLimitedDqElementTime = 0;
let toJSONTime = 0;
...
const nodeSerializer = {
  ...
  dqElmToSpec(dqElm, runOptions) {
    ...
    if (runOptions) {
      cloneLimitedDqElementStart = window.performance.now();
      dqElm = cloneLimitedDqElement(dqElm, runOptions);
      cloneLimitedDqElementTime += window.performance.now() - cloneLimitedDqElementStart;
    }
    ...
    toJSONStart = window.performance.now();
    const json = dqElm.toJSON();
    toJSONTime += window.performance.now() - toJSONStart;
...
nodeSerializer._logTime = function() {
  performanceTimer._log('Measure reporter#processAggregate_trimElementSpec_nodeSerializer_cloneLimitedDqElement took ' + cloneLimitedDqElementTime + 'ms');
  performanceTimer._log('Measure reporter#processAggregate_trimElementSpec_nodeSerializer_toJSON took ' + toJSONTime + 'ms');
  performanceTimer._log('Measure reporter#processAggregate_trimElementSpec_nodeSerializer_cloneSelector took ' + cloneSelectorTime + 'ms');
  performanceTimer._log('Measure reporter#processAggregate_trimElementSpec_nodeSerializer_cloneAncestry took ' + cloneAncestryTime + 'ms');
  performanceTimer._log('Measure reporter#processAggregate_trimElementSpec_nodeSerializer_cloneXpath took ' + cloneXpathTime + 'ms');
  performanceTimer._log('Measure reporter#processAggregate_trimElementSpec_nodeSerializer_DqElement took ' + DqElementTime + 'ms');
...
let cloneSelectorStart;
let cloneAncestryStart;
let cloneXpathStart;
let DqElementStart;

let cloneSelectorTime = 0;
let cloneAncestryTime = 0;
let cloneXpathTime = 0;
let DqElementTime = 0;
...
function cloneLimitedDqElement(dqElm, runOptions) {
  ...
  cloneSelectorStart = window.performance.now();
  const selector = hasSelectors ? dqElm.selector : [':root'];
  cloneSelectorTime += window.performance.now() - cloneSelectorStart;

  cloneAncestryStart = window.performance.now();
  const ancestry = hasAncestry ? dqElm.ancestry : [':root'];
  cloneAncestryTime += window.performance.now() - cloneAncestryStart;

  cloneXpathStart = window.performance.now();
  const xpath = hasXpath ? dqElm.xpath : '/';
  cloneXpathTime += window.performance.now() - cloneXpathStart;

  DqElementStart = window.performance.now();
  ...
  DqElementTime += window.performance.now() - DqElementStart;
