...
function processMessage(message, data) {
  if (!message) {
	...
	if (typeof message === 'object' && typeof data === 'object' && message !== null && data !== null) {
		const messageEmpty = Object.keys(message).length === 0;
		const dataEmpty = Object.keys(data).length === 0;
		const messageNotArray = !Array.isArray(message);
		const dataNotArray = !Array.isArray(data);
		if (messageEmpty && dataEmpty && messageNotArray && dataNotArray) {
			return;
		}
	}
