...
function substitute(str, data) {
  ...
  for (const prop in data) {
    if (data.hasOwnProperty(prop)) {
      ...
      str = str.replace(regex, toMessageString(data[prop]));
    }
  }
...
/**
 * Convert a data value to the text used in a message. Arrays are joined
 * with a comma and a space, like `${data.values}`.
 * @param {*} value
 * @return {String}
 */
