...
(function(exports) {
	...
	function verify(postedMessage) {
		if (
			...
			const messageSource = _getSource();
			return (
	...
	function publish(source, data, keepalive) {
		...
		if (subscriber) {
			const responder = createResponder(source, null, data.uuid);
	...
	if (typeof window.addEventListener === 'function') {
		window.addEventListener(
			...
			function(e) {
				...
				const keepalive = data._keepalive;
				const callback = messages[uuid];

				if (callback) {
					const result = data.error || data.message;
					const responder = createResponder(e.source, data.topic, uuid);
