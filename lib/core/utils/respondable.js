...
(function (exports) {
	...
	window.addEventListener('message', function (e) {
		...
		if (typeof data.error === 'object') {
			var msg;
			var ErrType = window[data.error.type] || Error;
			if (!data.error.stack) {
				msg = data.error.message || 'Unknown error occurred';
			} else {
				msg = data.error.stack.split(/: /)[1];
			}
			err = new ErrType(msg);
		}

		if (messages[uuid]) {
			var result = err || data.message;
			var responder = createResponder(e.source, data.topic, uuid);
			messages[uuid](result, keepalive, responder);
