...
const DqElement = function DqElement(elm, options, spec) {
  ...
  if (!axe._audit.noHtml) {
    getSourceStart = window.performance.now();
    this.source = this.spec.source ?? getSource(this._element);
    getSourceTime += window.performance.now() - getSourceStart;
  }
...
DqElement._logTime = function() {
  performanceTimer._log('Measure reporter#processAggregate_trimElementSpec_nodeSerializer_DqElement_selector took ' + selectorTime + 'ms');
  performanceTimer._log('Measure reporter#processAggregate_trimElementSpec_nodeSerializer_DqElement_getSource took ' + getSourceTime + 'ms');
  performanceTimer._log('Measure reporter#processAggregate_trimElementSpec_nodeSerializer_DqElement_total took ' + DqElementTime + 'ms');
}
