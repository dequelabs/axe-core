...
function matchesAttributes(vNode, exp) {
  return (
    ...
    exp.attributes.every(att => {
      ...
      return matchesHtmlInputType(vNode, att, nodeAtt);
    })
  );
}

/**
 * HTML input `type` is an ASCII case-insensitive enumerated attribute, so CSS
 * attribute selectors in HTML documents match it case-insensitively.
 * Preserve case-sensitive matching for XHTML and for `type` on other elements
 * (for example `ol[type]`, whose values are case-sensitive).
 * @see https://html.spec.whatwg.org/multipage/semantics-other.html#case-sensitivity-of-selectors
 * @param {VirtualNode} vNode
 * @param {Object} att Parsed attribute selector
 * @param {String} nodeAtt Attribute value from the node
 * @return {Boolean}
 */
function matchesHtmlInputType(vNode, att, nodeAtt) {
  if (
    att.key !== 'type' ||
    att.type !== 'attrValue' ||
    vNode.props.nodeName !== 'input' ||
    vNode._isXHTML
  ) {
    return false;
  }

  return nodeAtt.toLowerCase() === att.value.toLowerCase();
}
