...
function matchesTag(node, exp) {
	return (
		...
		(exp.tag === '*' || node.elementNodeName.toLowerCase() === exp.tag)
	);
}
...
function matchesClasses(node, exp) {
	return (
		...
		exp.classes.reduce((result, cl) => {
			return (
				result &&
				(node.elementClassName && node.elementClassName.match(cl.regexp))
			);
		}, true)
	);
}
...
function matchesAttributes(node, exp) {
	return (
		...
		exp.attributes.reduce((result, att) => {
			var nodeAtt = node.elementGetAttribute(att.key);
