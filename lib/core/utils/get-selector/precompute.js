 ...
 * flatten-tree drops. Three drop-sites need coverage:
 *
 ...
 *      DOM is invisible.
 *
 *   2. `<slot>` elements themselves. Flatten-tree never creates a
 *      vNode for a slot — it grafts the slot's assigned (or
 *      fallback) children onto the slot's parent in the flat tree.
 *      The slot still exists in the real DOM and shows up in
 *      `shadowRoot.querySelectorAll('slot')`, so its tag, classes,
 *      and attributes affect uniqueness answers.
 *
 *   3. Slot fallback content when the slot has assigned nodes. When
 ...
 *      never visited.
 *
 * All three are invisible to axe's rule pipeline (`select(...)`
 * never returns them, no rule ever reports them), so they never get
 * a selector generated FOR them — but they still exist for
 ...
 * would be wrong).
 *
 * Drop-sites #2 and #3 are handled together: we phantom-walk the
 * whole `<slot>` subtree (the slot itself AND its fallback children)
 * as a unit, so `parentIdx` chains through the slot correctly and a
 * slot with a distinguishing feature enters the ancestor filter at
 * the right level.
 *
...
export function getSelectorData(domTree) {
  ...
  for (let h = 0; h < shadowHostList.length; h++) {
    ...
    // Also handle the second and third drop-sites: <slot> elements
    // themselves are never given a vNode by flatten-tree, and when a
    // slot has assigned nodes its own children (the fallback markup)
    // are dropped along with it. Both still show up in
    // `shadowRoot.querySelectorAll(...)`, so we need them in the
    // bitmaps. Phantom-recording the slot as a real ancestor of the
    // fallback content also keeps the ancestor chain intact — if we
    // parented the fallback at the slot's parent instead, a slot with
    // a distinguishing feature (e.g. `<slot class="wrap">`) wouldn't
    // enter the candidate filter at level 1 and the walk would
    // terminate one step early.
    //
    // Fallback content lives inside the shadow root, not in the host's
    // light DOM, so its rootId is the shadow root's, not the host's.
    // The walk already assigned that rootId when any shadow-scoped
    // element was visited.
    ...
    for (let s = 0; s < slots.length; s++) {
      ...
      if (slot.assignedNodes().length === 0) {
        // Empty slot — flatten-tree walked the fallback children in
        // its place, so they're already in `_elmToIdx`.
        continue;
      }
      // Parent for the slot itself: its parent element inside the
      // shadow tree if we walked it, else the host (flatten-tree
      // grafts top-of-shadow content onto the host as its virtual
      // parent). `slot.parentElement` returns null when the slot is
      // directly under the shadow root — check for that explicitly
      // rather than relying on `??`, which would keep a `null` from
      // `_elmToIdx.get(...)` and let it coerce to `0` in `_parentOf`.
      let slotParentIdx = hostIdx;
      if (slot.parentElement) {
        const idx = data._elmToIdx.get(slot.parentElement);
        if (idx !== undefined) {
          slotParentIdx = idx;
        }
      }
      // `phantomWalk` recurses depth-first, so this single call
      // records the slot itself AND its fallback subtree with each
      // descendant's `parentIdx` pointing at the phantom above it.
      phantomWalk(slot, slotParentIdx, shadowRootId);
    }
  }
