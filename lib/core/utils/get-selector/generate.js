...
function computeFragment(elm, idx, selectorData) {
  ...
  // `:nth-child(N)` to disambiguate. The QSA-seed pass in
  // `precompute.js` puts every DOM sibling into `_elmToIdx` (real or
  // phantom), so every `sibIdx` lookup here resolves and the check is
  // always the precomputed one — no per-sibling CSS-engine call.
  ...
  if (siblings) {
    ...
    for (let i = 0; i < siblings.length; i++) {
      ...
      if (
        sibIdx !== undefined &&
        fragmentMatchesIdx(fragmentNoNth, sibIdx, perNode)
      ) {
        nthChild = -1;
      }
