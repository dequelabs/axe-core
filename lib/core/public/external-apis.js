...
const GET_ELEMENT_INTERNALS_DEFAULT_TIMEOUT = 1000;
...
export default function externalAPIs({
  ...
  if (isNotNullOrUndefined(internalsTimeout)) {
    assert(
      ...
      'getElementInternalsTimeout must be a number'
    );
    getElementInternalsTimeout = internalsTimeout;
  }
  // reset if set to null
  else if (internalsTimeout === null) {
    getElementInternalsTimeout = GET_ELEMENT_INTERNALS_DEFAULT_TIMEOUT;
  }
