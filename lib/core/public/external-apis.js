import { shadowSelect, getNodeFromTree, clone } from '../utils';
import log from '../log';

let getElementInternals;
let getElementInternalsTimeout;

export default function externalAPIs({
  getElementInternalsTimeout: internalsTimeout = 1000,
  getElementInternals: getInternals
} = {}) {
  getElementInternalsTimeout = internalsTimeout;

  if (getInternals !== null && getInternals !== undefined) {
    if (typeof getInternals !== 'function') {
      throw new TypeError(
...
/**
 * Set element internals map data provided by the user to elementInternals property on vNodes
 */
async function setElementInternals(logger = log) {
  if (!getElementInternals) {
    return;
  }

  const promiseValue = await Promise.race([
    asyncTimeout(getElementInternalsTimeout),
    getElementInternals()
  ]);

  if (promiseValue === 'timeout') {
    throw new Error('Timeout called for elementInternals');
  }
