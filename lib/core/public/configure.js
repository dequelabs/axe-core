...
function configure(spec) {
  ...
  if (spec.selectorConfig) {
    if (typeof spec.selectorConfig !== 'object') {
      throw new TypeError('selectorConfig property must be an object');
    }
    if (
      spec.selectorConfig.ignoredAttributes &&
      !Array.isArray(spec.selectorConfig.ignoredAttributes)
    ) {
      throw new TypeError('ignoredAttributes property must be an array');
    }
    audit.selectorConfig = {
      ...audit.selectorConfig,
      ...spec.selectorConfig
    };
  }
