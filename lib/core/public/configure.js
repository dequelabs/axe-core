...
function configureChecksRulesAndBranding(spec) {
	...
	if (spec.axeVersion) {
		let [major, minor, patch] = spec.axeVersion.split('.').map(Number);
		let [axeMajor, axeMinor, axePatch] = axe.version.split('.').map(Number);
		...
		if (
			...
			throw new Error(
				'Configured version is not compatible with current axe version'
			);
		}
	}
