...
export function setupGlobals(context) {
  ...
  if (!hasDoc) {
    ...
    cache.pull('globalDocumentSet', () => true);
    document = context.ownerDocument;
  }

  if (!hasWindow) {
    // cache.set('globalWindowSet', true);
    cache.pull('globalWindowSet', () => true);
...
export function resetGlobals() {
  // if (cache.pull('globalDocumentSet')) {
  // cache.set('globalDocumentSet', false);
  cache.pull('globalDocumentSet', () => false);

  document = null;
  // }
  // if (cache.pull('globalWindowSet')) {
  cache.pull('globalWindowSet', () => false);
