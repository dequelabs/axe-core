	...
	q.then(function (data) {
		try {
			...
			var results = axe.utils.mergeResults(data.map(function (d) {
				if (options.performanceTimer) {
					axe.utils.performanceTimer.mark('mark_results');
				}
				return {
					results: d
				};
			...
			if (options.performanceTimer) {
				axe.utils.performanceTimer.measure('audit_start_to_end', 'mark_audit_start', 'mark_audit_end');
				axe.utils.performanceTimer.measure('audit_start_to_results', 'mark_audit_start', 'mark_results');
				axe.utils.performanceTimer.getMeasures();
			}
