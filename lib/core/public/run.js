...
axe.run = function (context, options, callback) {
	...
	*/
	...
	// bolt on fetching preload assets if passed via options
	if (typeof options.preload !== 'undefined') {
		if (options.preload.assets !== 'undefined' &&
			Array.isArray(options.preload.assets) &&
			options.preload.assets.length > 0) {
			// let requestedAssets = options.preload.assets;
			// let requestedAssetsTimeout = axe.contants.preloadDefaultTimeout;
			// trigger a util to start fetching...
		} else {
			throw new Error('preload configuration property is wrongly set-up.');
			//TODO:JEY: document preload object in run config/ options
		}
	}
