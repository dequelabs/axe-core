...
const htmlElms = {
  ...
  figure: {
    variant: {
      // A figure with a child figcaption is restricted to its implicit
      // role (figure) and the DPub role doc-example.
      // See https://github.com/w3c/html-aria/pull/415
      figcaption: {
        matches: {
          condition: vNode =>
            !!vNode.children?.some(
              child => child.props.nodeName === 'figcaption'
            )
        },
        allowedRoles: ['doc-example']
      },
      // Otherwise any role is allowed (figure itself is NOT RECOMMENDED)
      default: {
        allowedRoles: true
      }
    },
