...
const htmlElms = {
  ...
  summary: {
    variant: {
      // A summary is the "summary for its parent details" when it is the
      // first summary child of a details element. Browsers expose it as the
      // details disclosure trigger, so authors cannot override its role.
      // See https://github.com/w3c/html-aria/pull/435
      summaryForDetails: {
        matches: {
          condition: vNode => {
            const parent = vNode.parent;
            if (!parent || parent.props.nodeName !== 'details') {
              return false;
            }
            return (
      ...
      // Otherwise the summary is exposed as generic, so any role is allowed.
      default: {
        allowedRoles: true
      }
    },
