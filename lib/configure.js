/*global Audit, runRules, runClassifier, runAnalysis, Rule, Classifier, AnalysisRule */
...
dqre.configure = function (audit) {
	...
	for (i = 0, l = audit.classifiers.length; i < l; i++) {
		dqre.audit.classifiers[audit.classifiers[i].id] = new Classifier(audit.classifiers[i]);
	}
	for (i = 0, l = audit.analyzers.length; i < l; i++) {
		dqre.audit.analyzers[audit.analyzers[i].id] = new AnalysisRule(audit.analyzers[i]);
	}
	...
	utils.respondable.subscribe('dqre.start', function (data, respond) {
		...
		if (data.command === 'rules') {
			return runRules(context, options, respond);
		}

		if (data.command === 'classify') {
			return runClassifier(data.parameter, context, options, respond);
		}

		if (data.command === 'analysis') {
			return runAnalysis(data.parameter, data.selectorArray, options, respond);
		}
	});
};
