...
text.accessibleText = function (element, inLabelledByContext, inControlContext) {
	...
	} else if (!(inControlContext && text.isEmbeddedControl(element)) && element.hasAttribute('aria-label')) {
		returnText += ' ' + text.sanitize(element.getAttribute('aria-label'));
	} else if (!(inControlContext && text.isEmbeddedControl(element)) && element.getAttribute('role') !== 'presentation') {
