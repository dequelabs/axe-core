/*global dom, color */
/* jshint maxstatements: 26, maxcomplexity: 11 */
...
/**
 * Returns the element that is visually "above" this one in z-index order where
 * supported at the position given inside the top-left corner of the provided
 * rectangle. Where not supported (IE < 10), returns the DOM parent.
 * @param {Element} node
 * @param {DOMRect} rect rectangle containing dimensions to consider
 * @return {Element}
 */
var getVisualParent = function (node, rect) {
	var visualParents,
		thisIndex,
		parent = null;
...
dom.getBackgroundColor = function (node, bgNodes) {
	...
	while (bgColor.alpha !== 1) {
		parent = getVisualParent(currentNode, rect);
		
		//Assume white if top level is not specified
		if (parent === null || parent.nodeType === 9) {
