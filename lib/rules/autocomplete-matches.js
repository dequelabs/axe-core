...
import { getAriaValue, getExplicitRole } from '../commons/aria';
...
function autocompleteMatches(node, virtualNode) {
  ...
  const ariaReadonly =
    getAriaValue(virtualNode, 'aria-readonly', { lowercase: true })?.value ??
    'false';
  if (virtualNode.hasAttr('readonly') || ariaReadonly === 'true') {
    return false;
  }
  ...
  const ariaDisabled =
    getAriaValue(virtualNode, 'aria-disabled', { lowercase: true })?.value ??
    'false';
