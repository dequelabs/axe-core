 ...
 */
function findHelp(type, id, check) {
	'use strict';
	function ch() { return ''; }

	return ((dqreConfiguration.messages[type + 'Help'] || {})[id] || ch)(check);
}
...
function failureSummary(result, nodeData) {
	...
	if (result === dqre.constants.result.FAIL) {
		nodeData.checks.forEach(function (check) {
			var help = findHelp('check', check.id, check);
