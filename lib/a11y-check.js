...
function failureSummary(result, nodeData) {
	...
	if (result === dqre.constants.result.FAIL) {
		...
		if (fails.length > 0) {
			...
			failMessage = dqreConfiguration.data.failureSummaries[dqre.constants.type.FAIL].failureMessage(it);
		}
	}
	...
	if (!hasPassingPass && failingPasses.length > 0) {
		...
		var p = dqreConfiguration.data.failureSummaries[dqre.constants.type.PASS].failureMessage(it);
		if (failMessage) {
			failMessage += "\n";
		}
		failMessage += p;
