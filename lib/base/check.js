...
function Check(spec) {
	...
	/**
	 * Free-form options that are passed as the second parameter to the `fn`
	 * @type {Mixed}
	 */
...
/**
 * Run the check (call `this.fn(node, this.options`)
 * @param  {HTMLElement}   node     The node to test
 * @param  {Function} callback Function to fire when check is complete
 */
Check.prototype.run = function (node, callback) {
	'use strict';
	if (this.matches(node)) {
		...
		try {
			result = this.fn.call(boundFn, node, this.options);
		} catch (e) {
			checkResult.error =  {
				message: e.message,
				stack: e.stack
