...
function parseSelectorArray(context, type) {
	...
	console.log(result);
/*
	var result = Array.prototype.concat.apply([],
		context[type].map(function (item) {
			// selector
			if (typeof item === 'string') {
				return utils.toArray(document.querySelectorAll(item));
			...
			if (item && item.length) {
				if (item.length > 1) {
					...
					return utils.toArray(document.querySelectorAll(item[0]));
				}
				return null;
			// node reference
			}
