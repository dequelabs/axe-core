...
export default function colorContrastEvaluate(node, options, virtualNode) {
  ...
  // Wrap the color analysis in a try-catch so that a single problematic
  // element produces an "incomplete" result rather than crashing the
  // entire audit (e.g. elements outside grid bounds due to CSS transforms,
  // negative margins, absolute positioning, etc.)
  try {
    return analyzeContrastColor.call(this, node, options, {
      expected,
      minThreshold,
      maxThreshold,
      bold,
    ...
    this.data({ messageKey: 'unableToAnalyze' });
    incompleteData.clear();
    return undefined;
  }
}

function analyzeContrastColor(node, options, checkData) {
  const { shadowOutlineEmMax, ignoreLength } = options;
  const { expected, minThreshold, maxThreshold, bold, fontSize, visibleText } =
    checkData;
