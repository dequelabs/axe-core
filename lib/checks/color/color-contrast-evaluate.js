...
export default function colorContrastEvaluate(node, options, virtualNode) {
  ...
  // getBackgroundColor walks the virtual grid to find elements behind the
  // text. TypeErrors here indicate the element's position fell outside the
  // grid (CSS transforms, negative margins, etc.) — report as incomplete
  // rather than crashing the entire audit.
  let bgNodes, bgColor, fgColor;
  try {
    bgNodes = [];
    bgColor = getBackgroundColor(node, bgNodes, shadowOutlineEmMax);
    fgColor = getForegroundColor(node, false, bgColor, options);
  } catch (error) {
