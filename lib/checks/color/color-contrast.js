...
let reasonData = color.incompleteData.get('bgColor');
// check if the missing reason was related to pseudo elements, and if so, pass along which one
let elementType = null;
if (reasonData && reasonData.indexOf('pseudo') >= 0) {
	elementType = color.incompleteData.get('elementType');
}
if (equalRatio && reasonData === undefined) {
