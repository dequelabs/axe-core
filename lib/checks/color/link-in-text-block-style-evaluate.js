import { getComposedParent, isInTextBlock } from '../../commons/dom';
...
export default function linkInTextBlockStyleEvaluate(node) {
  ...
  // Explain the length comparison that made the rule apply
  const { parentTextLength, widgetTextLength } = isInTextBlock(node, {
    returnLengths: true
  });
  this.data({ parentTextLength, widgetTextLength });
  return false;
}
