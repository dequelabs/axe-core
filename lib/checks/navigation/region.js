...
if (regionlessNodes) {
	if (regionlessNodes.includes(node)) {
		return false;
	}
...
function findRegionlessElms(virtualNode) {
	...
	if (
		...
		// Mark each parent node as having region child
...
regionlessNodes = findRegionlessElms(virtualNode)
...
this.data(regionlessNodes);
axe._cache.set('regionlessNodes', regionlessNodes);

return !regionlessNodes.includes(node);
