...
function isIdenticalResource(
	...
	return identicalNodes.every(({ data }) => {
		 ...
		 */
		if (nonMatchingKeys.includes('pathname')) {
			// there are other mis-matches than path/filename - fail
			if (nonMatchingKeys.length > 1) {
				return false;
			}
			// Either or both of the resource resulted in a load error
			if (
			...
			// Verify if document titles are the same
			if (resourceFrameTitle === expectedResourceFrameTitle) {
				return true;
			}
			return false;
		}
