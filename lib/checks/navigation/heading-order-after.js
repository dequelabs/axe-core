...
export default function headingOrderAfter(results) {
  ...
  results.forEach(result => {
    ...
    if (result.result === false) {
      addPreviousHeading(result, headingOrder, results);
    }
  });
  return results;
}
...
/**
 * Add the heading before a failing heading to its related nodes, so that the
 * skipped level can be found without looking through the whole page
 */
function addPreviousHeading(result, headingOrder, results) {
  const index = findHeadingOrderIndex(headingOrder, result.node.ancestry);
  const prevHeading = headingOrder[index - 1];
  const prevResult = results.find(({ node }) =>
    matchAncestry(node.ancestry, prevHeading.ancestry)
  );
  if (prevResult) {
    result.relatedNodes = [prevResult.node];
  }
}
