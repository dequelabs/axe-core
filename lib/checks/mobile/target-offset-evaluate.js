...
export default function targetOffsetEvaluate(node, options, vNode) {
  ...
  // Neighbors at least minOffset away from the target can not be too close
  const { left, top, width, height } = vNode.boundingClientRect;
  const offsetArea = new window.DOMRect(
    left - minOffset,
    top - minOffset,
    width + minOffset * 2,
    height + minOffset * 2
  ...
  for (const vNeighbor of findNearbyElms(vNode, minOffset)) {
    if (
      !getIntersectionRect(offsetArea, vNeighbor.boundingClientRect) ||
      getRoleType(vNeighbor) !== 'widget' ||
