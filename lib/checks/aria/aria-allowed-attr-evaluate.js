...
export default function ariaAllowedAttrEvaluate(node, options, virtualNode) {
  ...
  // Some elements disallow ARIA attributes their role would otherwise allow,
  // e.g. a summary that is its details element's disclosure trigger must not
  // use aria-expanded/aria-pressed. See ARIA in HTML (w3c/html-aria#435).
  const { disallowedAriaAttrs } = getElementSpec(virtualNode);
  if (Array.isArray(disallowedAriaAttrs)) {
    allowed = allowed.filter(
      attrName => !disallowedAriaAttrs.includes(attrName)
    );
  }
