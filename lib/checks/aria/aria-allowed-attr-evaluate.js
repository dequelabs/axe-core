...
function ariaAllowedAttrEvaluate(node, options, virtualNode) {
  ...
  // gets parent (if the parent is a not a rowgroup)
  const parentTableRole = [...virtualNode.parent.actualNode.attributes][0]
    .value;

  for (let i = 0; i < attrs.length; i++) {
    const attrName = attrs[i];
    if (validateAttr(attrName) && parentTableRole === 'table') {
