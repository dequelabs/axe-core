...
export default function ariaAllowedAttrEvaluate(node, options, virtualNode) {
  ...
  for (const attrName of virtualNode.attrNames) {
    let isAllowed = allowed.includes(attrName);

    // allow aria-expanded for a button element but only if
    // popovertarget and command attributes are not present
    // @see https://github.com/dequelabs/axe-core/issues/4837
    if (attrName === 'aria-expanded') {
      const isButtonWithStandaloneAriaExpanded =
        role === 'button' &&
        !(
          virtualNode.hasAttr('popovertarget') || virtualNode.hasAttr('command')
        );
      isAllowed =
        allowed.includes(attrName) && isButtonWithStandaloneAriaExpanded;
    }
    ...
    if (
      ...
      console.log('adding to invalid array...', attrName);
