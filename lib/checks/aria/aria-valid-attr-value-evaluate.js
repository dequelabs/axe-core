...
function ariaValidAttrValueEvaluate(node, options, virtualNode) {
  ...
  const preChecks = {
    ...
    'aria-current': () => {
      if (!validateAttrValue(virtualNode, 'aria-current')) {
    ...
    'aria-describedby': () => {
      if (!validateAttrValue(virtualNode, 'aria-describedby')) {
    ...
    'aria-labelledby': () => {
      if (!validateAttrValue(virtualNode, 'aria-labelledby')) {
  ...
  for (let i = 0, l = attrs.length; i < l; i++) {
    ...
    try {
      if (
        ...
        !validateAttrValue(virtualNode, attrName)
      ) {
