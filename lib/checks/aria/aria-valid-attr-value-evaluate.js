...
function ariaValidAttrValueEvaluate(node, options, virtualNode) {
  ...
  virtualNode.attrNames.forEach(attrName => {
    // skip any attributes handled elsewhere (we don't want use
    // Array.filter first as that would mean looping over the
    // attributes twice. on a page with tens of thousands of
    // nodes that's a lot of overhead cost in performance)
    if (
