...
export default function ariaRequiredChildrenEvaluate(
  ...
  if (unallowed.length) {
    const listOfUnallowed = unallowed
  ...
  if (
    !reviewEmpty.includes(explicitRole) &&
    virtualNode.attr('aria-busy') === 'true' &&
    !ownedRoles.some(({ vNode }) => vNode.props.nodeType === 1)
  ) {
    return true;
  }
