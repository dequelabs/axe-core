...
export default function ariaRequiredChildrenEvaluate(
  ...
  const unallowed = ownedRoles.filter(
    ({ role, vNode }) =>
      ...
      !isAllowedInContext(role, explicitRole)
  );
...
/**
 * Check if a role is allowed in the parent role because the parent is one
 * of its required context roles, even though it is not a required owned
 * role (e.g. `caption` in a `table`).
 * @param {String|null} role
 * @param {String} parentRole
 * @return {Boolean}
 */
function isAllowedInContext(role, parentRole) {
  const context = requiredContext(role);
