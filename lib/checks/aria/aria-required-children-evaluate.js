...
function getOwnedRoles(virtualNode, required) {
  ...
  while (ownedVirtual.length) {
    const ownedElement = ownedVirtual.shift();
    const { props, children } = ownedElement;
    if (props.nodeType === 3) {
      ownedElements.push(ownedElement);
    }
    if (props.nodeType !== 1 || !isVisibleToScreenReaders(ownedElement)) {
      continue;
    }
...
function getGlobalAriaAttr(vNode) {
  return getGlobalAriaAttrs().find(
    attr => attr !== 'aria-hidden' && vNode.hasAttr(attr)
  );
}
