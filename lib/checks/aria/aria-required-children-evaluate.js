...
import { idrefs, hasContentVirtual } from '../../commons/dom';
import {
	matchesSelector,
	querySelectorAll,
	getNodeFromTree
...
function ariaRequiredChildrenEvaluate(node, options, virtualNode, context) {
	const reviewEmpty =
	...
	function ariaOwns(nodes, role) {
		...
		for (index = 0, length = nodes.length; index < length; index++) {
			...
			const virtualTree = getNodeFromTree(nodes[index]);
