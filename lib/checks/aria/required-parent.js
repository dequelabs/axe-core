...
function getMissingContext(virtualNode, requiredContext, includeElement) {
	...
	for (index = 0, length = requiredContext.length; index < length; index++) {
		...
		if (hasParentWithRole(virtualNode, context)) {
 ...
 * @returns {Boolean}
 */
function hasParentWithRole(vNode, role) {
	const parent = vNode.parent;
	if (!parent) {
		return false;
	}

	const parentRole = aria.getRole(parent);
	if (
		(!parentRole || parentRole === `presentation`) &&
		parent.actualNode.nodeType === 1
	) {
