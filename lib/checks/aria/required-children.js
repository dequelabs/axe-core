...
const getRoleInheritance = axe.commons.aria.getRoleInheritance;
...
function owns(node, virtualTree, role, ariaOwned) {
	...
	const roles = getRoleInheritance(role);
	let selector = [];
	roles.forEach(roleName => {
		const implicit = implicitNodes(roleName).map(
			implicitSelector => implicitSelector + ':not([role])'
		);
		selector.push('[role="' + roleName + '"]');
