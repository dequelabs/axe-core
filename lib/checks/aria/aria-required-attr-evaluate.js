...
export default function ariaRequiredAttrEvaluate(
  ...
  if (
    role === 'slider' &&
    getAriaValue(virtualNode, 'aria-valuetext')?.value?.trim()
  ) {
    return true;
  }

  const elmSpec = getElementSpec(virtualNode);
  const missingAttrs = requiredAttrs.filter(requiredAttr => {
    const attrValue = virtualNode.attr(requiredAttr);
    if (attrValue) {return false;} // non-empty HTML attr present
    if (hasImplicitAttr(elmSpec, requiredAttr)) {return false;}
    // when attr is completely absent from HTML, also check property/internals
    if (attrValue === null && standards.ariaAttrs[requiredAttr]) {
      return !hasAriaValue(virtualNode, requiredAttr);
    }
    return true; // empty/valueless HTML attr, or non-ARIA option attr
  });
