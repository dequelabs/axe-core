...
if (playableDuration <= options.allowedDuration && !node.hasAttribute('loop')) {
	return true;
}
...
function getPlayableDuration(elm) {
	/**
	 * get source
	 * 	`currentSrc` is populated with `src` is set on HTMLMediaElement
	 * 	 otherwise interpret src from `source` tags
	 */
	const src = elm.currentSrc
		? elm.currentSrc
		: Array.from(elm.getElementsByTagName('source')).find(({ src }) => !!src)
				.src;
	if (!src) {
		return 0;
	}

	const playbackRange = getPlaybackRange(src);
	if (!playbackRange) {
