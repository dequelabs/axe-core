...
function checkFieldset(group, name) {
	const firstNode = group.firstElementChild;
	...
	const otherElements = getUnrelatedElements(group, name);
...
function checkARIAGroup(group, name) {
	const hasLabelledByText = axe.commons.dom
...
const result = runCheck(virtualNode);
if (!result) {
