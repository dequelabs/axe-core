...
- `configurationOptions` - Options object; where the valid name, value pairs are:
  ...
  - `ver` - Set the compatible version of a custom rule with the current axe version. Compatible versions are all patch and minor updates that are the same as, or newer than those of the `ver` property.
