...
- [ ] Code built with `pnpm run build` before testing
- [ ] All applicable tests pass locally: `pnpm test` (unit), `test:integration`, `test:virtual-rules`, `test:act`, `test:apg`, `test:examples`, `test:node`, `test:jsdom`

## Formatting & Build

- [ ] `pnpm run fmt` passes (Prettier)
- [ ] `pnpm run eslint` passes
- [ ] `pnpm run build` run — generated artifacts that are tracked in git (for example `locales/_template.json`) are committed in the same commit as source changes
...
- [ ] `doc/rule-descriptions.md` is auto-generated by `pnpm run build` — verify it regenerates correctly for new rules
