describe('get-target-rects', () => {
  ...
  it('returns subset rect when obscured', () => {
    const vNode = queryFixture(html`
      <button
        id="target"
        style="width: 30px; height: 40px; position: absolute; left: 10px; top: 5px"
      >
        x
      </button>
      <div
        style="position: absolute; left: 30px; top: 0; width: 50px; height: 50px;"
      ></div>
    `);
    const rects = getTargetRects(vNode);
  ...
  it('ignores elements with "pointer-events: none"', () => {
    const vNode = queryFixture(html`
      ...
      <div
        style="position: absolute; left: 30px; top: 0; width: 50px; height: 50px; pointer-events: none"
      ></div>
    `);
    const rects = getTargetRects(vNode);
    assert.deepEqual(rects, [vNode.actualNode.getBoundingClientRect()]);
  ...
  it("ignores elements that don't overlap the target", () => {
    const vNode = queryFixture(html`
      ...
      <div
        style="position: absolute; left: 60px; top: 0; width: 50px; height: 50px;"
      ></div>
    `);
    const rects = getTargetRects(vNode);
    assert.deepEqual(rects, [vNode.actualNode.getBoundingClientRect()]);
  ...
  it('ignores non-tabbable descendants of the target', () => {
    const vNode = queryFixture(html`
      ...
      >
        <div
          style="position: absolute; left: 5px; top: 5px; width: 50px; height: 50px;"
        ></div>
      </button>
    `);
    const rects = getTargetRects(vNode);
  ...
  it('returns each unobscured area', () => {
    const vNode = queryFixture(html`
      ...
      ></div>
      <div
        style="position: absolute; left: 0px; top: 20px; width: 50px; height: 10px"
      ></div>
    `);
    const rects = getTargetRects(vNode);
