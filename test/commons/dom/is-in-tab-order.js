describe('dom.isInTabOrder', () => {
  const queryFixture = axe.testUtils.queryFixture;
  const isInTabOrder = axe.commons.dom.isInTabOrder;

  it('should return false for presentation element with negative tabindex', () => {
    const target = queryFixture('<div id="target" tabindex="-1"></div>');
    assert.isFalse(isInTabOrder(target));
  });

  it('should return true for presentation element with positive tabindex', () => {
    const target = queryFixture('<div id="target" tabindex="1"></div>');
    assert.isTrue(isInTabOrder(target));
  });

  it('should return false for presentation element with tabindex not set', () => {
    const target = queryFixture('<div id="target"></div>');
    assert.isFalse(isInTabOrder(target));
  });

  it('should return false for presentation element with tabindex set to non-parseable value', () => {
    const target = queryFixture('<div id="target" tabindex="foobar"></div>');
    assert.isFalse(isInTabOrder(target));
  });

  it('should return false for presentation element with tabindex not set and role of natively focusable element', () => {
    const target = queryFixture('<div id="target" role="button"></div>');
    assert.isFalse(isInTabOrder(target));
  });

  it('should return true for natively focusable element with tabindex 0', () => {
    const target = queryFixture('<button id="target" tabindex="0"></button>');
    assert.isTrue(isInTabOrder(target));
  });

  it('should return true for natively focusable element with tabindex 1', () => {
    const target = queryFixture('<button id="target" tabindex="1"></button>');
    assert.isTrue(isInTabOrder(target));
  });

  it('should return false for natively focusable element with tabindex -1', () => {
    const target = queryFixture('<button id="target" tabindex="-1"></button>');
    assert.isFalse(isInTabOrder(target));
  });

  it('should return true for natively focusable element with tabindex not set', () => {
    const target = queryFixture('<button id="target"></button>');
    assert.isTrue(isInTabOrder(target));
  });

  it('should return true for natively focusable element with tabindex set to empty string', () => {
    const target = queryFixture('<button id="target" tabindex=""></button>');
    assert.isTrue(isInTabOrder(target));
  });

  it('should return true for natively focusable element with tabindex set to non-parseable value', () => {
  ...
  it('should return false for disabled natively focusable element with tabindex', () => {
    const target = queryFixture(
  ...
  it('should return false for natively focusable hidden element', () => {
    const target = queryFixture('<button id="target" hidden></button>');
    assert.isFalse(isInTabOrder(target));
  });

  it('should return for false hidden element with tabindex 1', () => {
