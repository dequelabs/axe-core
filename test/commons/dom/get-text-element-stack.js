describe('dom.getTextElementStack', () => {
  const html = axe.testUtils.html;

  const fixture = document.getElementById('fixture');
  const getTextElementStack = axe.commons.dom.getTextElementStack;
  ...
  it('should return array of client text rects', () => {
    fixture.innerHTML = html`
      <main id="1">
        <div id="target"><span id="2">Hello</span><br />World</div>
      </main>
    `;
    axe.testUtils.flatTreeSetup(fixture);
    const target = fixture.querySelector('#target');
    const stacks = getTextElementStack(target).map(mapToIDs);
    assert.deepEqual(stacks, [['target', '1', 'fixture']]);
  });

  it('should ignore newline characters', () => {
    fixture.innerHTML = html`
      <main id="1">
        <div id="target">
    ...
    fixture.innerHTML = html`
      <main id="1">
        ...
        >
          ...
          sollicitudin quam. Fusce mi odio, egestas pulvinar erat eget, vehicula
          ...
          bibendum ligula, at tempor felis ante non libero. Donec dapibus, diam
          sit amet posuere commodo, magna orci hendrerit ipsum, eu egestas
          mauris nulla ut ipsum. Sed luctus, orci in fringilla finibus, odio leo
          porta dolor, eu dignissim risus eros eget erat. World
        </div>
      </main>
    `;
    axe.testUtils.flatTreeSetup(fixture);
  ...
  it('should handle text that is too large for the container', () => {
    fixture.innerHTML = html`
  ...
  it('should handle text that overflows outside the parent', () => {
    fixture.innerHTML = html`
      <div id="1" style="width: 250px; overflow: hidden">
        <p
          id="target"
          style="max-height: 80px; overflow: hidden; line-height: 20px; font-size: 13px;"
        >
          The Chandni Chowk (Moonlight Square) is one of the oldest and busiest
          markets in Old Delhi, India. Chandni Chowk is located close to Old
          Delhi Railway Station. The Red Fort monument is located within the
          market. It was built in the 17th century by Mughal Emperor of India
          Shah Jahan and designed by his daughter Jahanara. The market was once
          divided by canals (now closed) to reflect moonlight and remains one of
          India's largest wholesale markets.
        </p>
      </div>
    `;
    axe.testUtils.flatTreeSetup(fixture);
