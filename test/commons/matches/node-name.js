describe('matches.nodeName', () => {
  const matchNodeName = axe.commons.matches.nodeName;
  ...
  it('returns true if the nodeName is the same as the matcher', () => {
    const virtualNode = queryFixture('<h1 id="target">foo</h1>');
    assert.isTrue(matchNodeName(virtualNode, 'h1'));
  });

  it('returns true if the nodename is included in an array', () => {
    const virtualNode = queryFixture('<h1 id="target">foo</h1>');
    assert.isTrue(matchNodeName(virtualNode, ['h3', 'h2', 'h1']));
  });

  it('returns true if the nodeName matches a regexp', () => {
    const virtualNode = queryFixture('<h1 id="target">foo</h1>');
    assert.isTrue(matchNodeName(virtualNode, /^h[0-6]$/));
  });

  it('returns true if the nodeName matches with a function', () => {
    const virtualNode = queryFixture('<h1 id="target">foo</h1>');
    assert.isTrue(
      matchNodeName(virtualNode, nodeName => {
        return nodeName === 'h1';
      })
    );
  });

  it('returns false if the nodeName does not match', () => {
