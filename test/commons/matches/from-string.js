describe('matches.fromString', function () {
  ...
  describe('with void matchers', function () {
    it('returns true for null', function () {
      assert.isTrue(fromString('foo', null))
    })
    it('returns true for undefined', function () {
      assert.isTrue(fromString('foo', undefined))
    })
  })
  ...
  describe('with function matchers', function () {
    it('returns true if the function returns a truthy value', function () {
      assert.isTrue(fromString('foo', function (val) {
        assert.equal(val, 'foo')
        return true
      }))
      assert.isTrue(fromString('foo', function () {
        return 123
      }))
      assert.isTrue(fromString('foo', function () {
        return {}
    ...
    it('returns false if the function returns a falsey value', function () {
      assert.isFalse(fromString('foo', function (val) {
        assert.equal(val, 'foo')
        return false
      }))
      assert.isFalse(fromString('foo', function () {
        return 0
      }))
      assert.isFalse(fromString('foo', function () {
        return undefined
