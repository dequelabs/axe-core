describe('aria.isValidRole', function() {
	...
	it('returns true for abstract roles by default', function() {
		assert.isTrue(axe.commons.aria.isValidRole('input'));
	});

	it('returns false for abstract roles with { disallowAbstract: true } ', function() {
		assert.isFalse(
			axe.commons.aria.isValidRole('input', { disallowAbstract: true })
		);
	});
});
