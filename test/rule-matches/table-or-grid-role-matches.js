describe('table-or-grid-role-matches', () => {
  ...
  ['table', 'grid', 'treegrid'].forEach(role => {
    it(`returns true for tables with role=${role}`, () => {
      const vNode = queryFixture(
        html`<table id="target" role="${role}">
          <th id="h">foo</th>
          <td headers="h">bar</td>
        </table>`
      );
  ...
  it(`returns true for tables with an invalid role`, () => {
    const vNode = queryFixture(
      html`<table id="target" role="invalid-aria-role">
        <th id="h">foo</th>
        <td headers="h">bar</td>
      </table>`
    );
  ...
  it(`returns true for focusable tables with role=none`, () => {
    const vNode = queryFixture(
      html`<table id="target" role="none" tabindex="0">
        <th id="h">foo</th>
        <td headers="h">bar</td>
      </table>`
    );
  ...
  it(`returns true for tables with role=none but with a global ARIA attribute`, () => {
    const vNode = queryFixture(
      html`<table id="target" role="none" aria-live="assertive">
        <th id="h">foo</th>
        <td headers="h">bar</td>
      </table>`
    );
