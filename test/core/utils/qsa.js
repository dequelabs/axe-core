...
function getTestDom() {
  ...
  let tree = axe.utils.getFlattenedTree(html);

  // setup shadowIds for testing
  let first = axe.utils.getNodeFromTree(html.querySelector('.first'));
  let second = axe.utils.getNodeFromTree(html.querySelector('.second'));
  setShadowId(first, 'a');
...
describe('axe.utils.querySelectorAllFilter', function () {
  ...
  let tests = ['without cache', 'with cache'];
  for (let i = 0; i < tests.length; i++) {
    let describeName = tests[i];
    describe(describeName, function () {
      ...
      if (describeName === 'without cache') {
        ...
        it('should not use the cache if not using the top-level node', function () {
          let nodes = axe.utils.querySelectorAllFilter(dom, 'ul');

          // this would return 4 nodes if we were still using the
          // top-level cache
          let result = axe.utils.querySelectorAllFilter(nodes[0], 'li');
          assert.equal(result.length, 2);
        });
      }

      it('should find nodes using just the tag', function () {
        let result = axe.utils.querySelectorAllFilter(dom, 'li');
        assert.equal(result.length, 4);
      });
      it('should find nodes using parent selector', function () {
        let result = axe.utils.querySelectorAllFilter(dom, 'ul > li');
        assert.equal(result.length, 4);
      });
      it('should NOT find nodes using parent selector', function () {
        let result = axe.utils.querySelectorAllFilter(dom, 'div > li');
        assert.equal(result.length, 0);
      });
      it('should find nodes using nested parent selectors', function () {
        let result = axe.utils.querySelectorAllFilter(
          dom,
          'span > span > span > span'
        );
        assert.equal(result.length, 2);
      ...
      it('should find nodes using class selector', function () {
        let result = axe.utils.querySelectorAllFilter(dom, '.breaking');
        assert.equal(result.length, 2);
      });
      it('should find nodes using hierarchical class selector', function () {
        let result = axe.utils.querySelectorAllFilter(dom, '.first .breaking');
        assert.equal(result.length, 2);
      });
      it('should NOT find nodes using hierarchical class selector', function () {
        let result = axe.utils.querySelectorAllFilter(dom, '.second .breaking');
        assert.equal(result.length, 0);
      });
      it('should find nodes using multiple class selector', function () {
        let result = axe.utils.querySelectorAllFilter(dom, '.second.third');
        assert.equal(result.length, 1);
      });
      it('should find nodes using id', function () {
        let result = axe.utils.querySelectorAllFilter(dom, '#one');
        assert.equal(result.length, 1);
      });
      ...
      it('should find nodes using attribute', function () {
        let result = axe.utils.querySelectorAllFilter(dom, '[role]');
        assert.equal(result.length, 2);
      });
      it('should find nodes using attribute with value', function () {
        let result = axe.utils.querySelectorAllFilter(dom, '[role=tab]');
        assert.equal(result.length, 1);
      });
      it('should find nodes using attribute with value', function () {
        let result = axe.utils.querySelectorAllFilter(dom, '[role="button"]');
        assert.equal(result.length, 1);
      });
      ...
      it('should find nodes using :not selector with class', function () {
        let result = axe.utils.querySelectorAllFilter(dom, 'div:not(.first)');
        assert.equal(result.length, 2);
      });
      it('should find nodes using :not selector with matching id', function () {
        let result = axe.utils.querySelectorAllFilter(dom, 'div:not(#one)');
        assert.equal(result.length, 2);
      });
      ...
      it('should find nodes using :not selector with bogus id', function () {
        let result = axe.utils.querySelectorAllFilter(dom, 'div:not(#thangy)');
        assert.equal(result.length, 3);
      });
      it('should find nodes using :not selector with attribute', function () {
        let result = axe.utils.querySelectorAllFilter(dom, 'div:not([id])');
        assert.equal(result.length, 2);
      });
      ...
      it('should find nodes using ^= attribute selector', function () {
        let result = axe.utils.querySelectorAllFilter(dom, '[class^="sec"]');
        assert.equal(result.length, 1);
      });
      it('should find nodes using $= attribute selector', function () {
        let result = axe.utils.querySelectorAllFilter(dom, '[id$="ne"]');
        assert.equal(result.length, 3);
      });
      it('should find nodes using *= attribute selector', function () {
        let result = axe.utils.querySelectorAllFilter(dom, '[role*="t"]');
        assert.equal(result.length, 2);
      });
      ...
      it('should find an element only once', function () {
        let divs = axe.utils.querySelectorAllFilter(dom, 'div');
        let ones = axe.utils.querySelectorAllFilter(dom, '#one');
        let divOnes = axe.utils.querySelectorAllFilter(dom, 'div, #one');
      ...
      it('should return nodes sorted by document position', function () {
        let result = axe.utils.querySelectorAllFilter(dom, 'ul, #one');
...
describe('axe.utils.querySelectorAll', function () {
  ...
  it('should find nodes using just the tag', function () {
    let result = axe.utils.querySelectorAll(dom, 'li');
    assert.equal(result.length, 4);
  });
});
