describe('no-focusable-content tests', function () {
  ...
  let noFocusableContent = axe.testUtils.getCheckEvaluate(
    'no-focusable-content'
  );
  let checkSetup = axe.testUtils.checkSetup;
  let check = checks['no-focusable-content'];
  ...
  it('should return true if element has no focusable content', function () {
    let vNode = queryFixture('<button id="target"><span>Hello</span></button>');
  ...
  it('should return true if element only has text content', function () {
    let vNode = queryFixture('<button id="target">Hello</button>');
    assert.isTrue(noFocusableContent(null, null, vNode));
  });
