...
checks['aria-labelledby'] = {
	title: 'Has an aria-labelledby',
	fn: function (node) {
		var result = felib.dom.idrefs(node, 'aria-labelledby');
		return {
			result: result.indexOf(null) === -1 && !!result.length,
			data: result.map(function (thing) {
				return felib.text.visible(thing, true);
			}).join('')
		};
	}
};
...
checks['implicit-label'] = {
	...
	fn: function (node) {
		...
		return {
			result: label !== null,
			data: label && felib.text.visible(label, true)
		};
	}
};

checks['explicit-label'] = {
	title: 'label for stuff',
	selector: '[id]',
	fn: function (node) {
		var label = document.querySelector('label[for="' + node.id + '"]');
		return {
			result: label !== null,
			data: felib.text.visible(label, true)
		};
	}
};
